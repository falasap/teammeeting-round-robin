...
function Icon({
  ...
  const handleClick2 = React11.useCallback(
    (e) => {
  ...
  if (!pathData19 || pathData19.length === 0) {
    return null;
  }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("g", { role: "presentation", children: pathData19.map((path, index) => /* @__PURE__ */ jsxRuntime.jsx("path", { d: path }, index)) })
      ]
    }
  );
}
...
var ResizeCornerIcon = Object.assign(
  React11__namespace.default.forwardRef(
...
function Popover({
  ...
  const focusedBeforeOpen = React11.useRef(null);
  const [isOpen, setIsOpen] = React11.useState(false);
  const [pos, setPos] = React11.useState(null);
  const [resizeSize, setResizeSize] = React11.useState(null);
  const resizeState = React11.useRef(null);
  const [isDesktop2] = React11.useState(() => isDesktop());
  const containerRef = React11.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React11.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React11.useRef(() => {
  });
  const reposition = React11.useCallback(() => {
  ...
  const focusLast = React11.useCallback(() => {
    if (!popoverRef.current) return;
    const el = getLastFocusable(popoverRef.current);
    (el || popoverRef.current).focus();
  }, []);
  const handleResizeMouseDown = React11.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React11.useMemo(() => {
...
function BusyIndicator({
  ...
  const [isBusy, setIsBusy] = React11.useState(false);
  const labelId = React11.useId();
  const hasChildren = React11__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React11.useRef(null);
  const [animationPhase, setAnimationPhase] = React11.useState("idle");
  const [displayedState, setDisplayedState] = React11.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React11.useState(void 0);
  ...
  const handleKeyUp = React11.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function useIsPhone() {
  const [phone, setPhone] = React11__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React11__namespace.useState(false);
  const dialogId = React11__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React11__namespace.useRef(null);
  const headerRef = React11__namespace.useRef(null);
  const contentRef = React11__namespace.useRef(null);
  const previousActiveElementRef = React11__namespace.useRef(null);
  const returnValueRef = React11__namespace.useRef(void 0);
  const isClosingRef = React11__namespace.useRef(false);
  const didPopRef = React11__namespace.useRef(false);
  const [isDragging, setIsDragging] = React11__namespace.useState(false);
  const [position, setPosition] = React11__namespace.useState(null);
  const positionRef = React11__namespace.useRef(position);
  React11__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React11__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React11__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React11__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React11__namespace.useCallback(() => {
  ...
  const clampPosition = React11__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React11__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React11__namespace.useRef(isPopoverActuallyOpen);
  React11__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React11__namespace.useRef(onAfterClose);
  React11__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React11__namespace.useRef(handleClose);
  React11__namespace.useEffect(() => {
  ...
  const handleMouseDown = React11__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React11__namespace.useCallback(
    (e) => {
  ...
  const resizeState = React11__namespace.useRef(null);
  const handleResizeMouseDown = React11__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
...
function useIsPhone2() {
  const [phone] = React11.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const handleMobileClose = React11.useCallback(() => {
...
var ListContext = React11__namespace.createContext(null);
function useOptionalListContext() {
  return React11.useContext(ListContext);
}
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React11.useState(new Set(defaultKeys));
  const previousKeysRef = React11.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React11.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React11.useCallback(() => {
  ...
  const setSelected = React11.useCallback((key, selected) => {
  ...
  const toggleSelection = React11.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React11.useCallback((key) => {
...
function useListNavigation({
  ...
  const getAllNavigable = React11.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React11.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const registerItem = React11.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListInner(props) {
  ...
  const growingButtonRef = React11.useRef(null);
  const beforeSentinelRef = React11.useRef(null);
  const afterSentinelRef = React11.useRef(null);
  const [showLoading, setShowLoading] = React11.useState(false);
  const [mediaRange, setMediaRange] = React11.useState("M");
  const itemRegistry = React11.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React11.useState(null);
  const [draggedKey, setDraggedKey] = React11.useState(null);
  const [dropTargetKey, setDropTargetKey] = React11.useState(null);
  const [dropPlacement, setDropPlacement] = React11.useState(null);
  ...
  const itemCount = React11.useMemo(() => {
    ...
    return React11__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React11.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React11.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React11.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React11.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React11.useCallback((key, e) => {
  ...
  const handleDrop = React11.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React11.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React11.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React11.useCallback((e) => {
  ...
  const getAllTabbableElements = React11.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React11.useCallback(() => {
  ...
  const enterListAtFocusedItem = React11.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React11.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React11.useMemo(() => {
  ...
  const handleLoadMore = React11.useCallback(() => {
  ...
  const renderedItems = React11.useMemo(() => {
    ...
    if (items && renderItem) {
      return items.map((item, index) => {
        ...
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React11__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React11__namespace.Children.forEach(children, (child) => {
        if (React11__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React11__namespace.memo(ListInner);
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    ResponsivePopover,
    {
      ...
      children: /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex flex-col", children: [
        hasValueState && /* @__PURE__ */ jsxRuntime.jsx("div", { ref: valueStatePortalRef }),
        loading ? /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex items-center justify-center py-8", children: [
          /* @__PURE__ */ jsxRuntime.jsx(lucideReact.Loader2, { className: "h-5 w-5 animate-spin text-sapphire-text-tertiary" }),
          /* @__PURE__ */ jsxRuntime.jsx("span", { className: "ml-2 text-sm text-sapphire-text-tertiary", children: "Loading..." })
...
var valueStateLabelKeys = {
  ["None" /* None */]: "",
  ["Positive" /* Positive */]: "VALUE_STATE_SUCCESS",
  ["Negative" /* Negative */]: "VALUE_STATE_ERROR",
  ["Critical" /* Critical */]: "VALUE_STATE_WARNING",
  ["Information" /* Information */]: "VALUE_STATE_INFORMATION"
};
...
function ValueStateMessage({
  ...
  if (isOpen && hasValueState && portalTarget) {
    return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
      /* @__PURE__ */ jsxRuntime.jsx("div", { style: { height: messageHeight }, className: "mt-2" }),
      reactDom.createPortal(
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              ValueStateIcon && /* @__PURE__ */ jsxRuntime.jsx(ValueStateIcon, { className: "h-4 w-4 shrink-0 mt-0.5" }),
              /* @__PURE__ */ jsxRuntime.jsx("span", { className: cn("text-sm", valueStateMessageColors[normalizedValueState]), children: valueStateMessage })
            ]
          }
        ),
        portalTarget
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        normalizedValueState !== "None" /* None */ && valueStateLabelKeys[normalizedValueState] && /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "sr-only", children: [
          t(valueStateLabelKeys[normalizedValueState]),
          ":",
          " "
        ] }),
        ValueStateIcon && /* @__PURE__ */ jsxRuntime.jsx(ValueStateIcon, { className: "h-4 w-4 shrink-0 mt-0.5" }),
        /* @__PURE__ */ jsxRuntime.jsx("span", { children: valueStateMessage })
      ]
    }
  );
}
function ClearButton({ onClick, accessibleName = "Clear value", comboBoxSize = "Large" /* Large */ }) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("span", { onMouseDown: (e) => e.preventDefault(), children: /* @__PURE__ */ jsxRuntime.jsx(
    Button,
    {
      design: "SecondaryNeutral" /* SecondaryNeutral */,
      size: isMedium ? "Small" /* Small */ : "Medium" /* Medium */,
      iconOnly: true,
      icon: /* @__PURE__ */ jsxRuntime.jsx("span", { className: cn("inline-flex items-center justify-center", isMedium ? "h-3 w-3" : "h-4 w-4"), children: /* @__PURE__ */ jsxRuntime.jsx(DeclineIcon, { className: "h-full w-full" }) }),
      tabIndex: -1,
      onClick,
      accessibleName,
      className: "shrink-0"
...
function DropdownArrowButton({ onClick, disabled = false, comboBoxSize = "Large" /* Large */ }) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("span", { onMouseDown: (e) => e.preventDefault(), children: /* @__PURE__ */ jsxRuntime.jsx(
    Button,
    {
      ...
      size: isMedium ? "Small" /* Small */ : "Medium" /* Medium */,
      iconOnly: true,
      icon: /* @__PURE__ */ jsxRuntime.jsx("span", { className: cn("inline-flex items-center justify-center", isMedium ? "h-3 w-3" : "h-4 w-4"), children: /* @__PURE__ */ jsxRuntime.jsx(SlimArrowDownIcon, { className: "h-full w-full" }) }),
      tabIndex: -1,
      onClick,
...
function useResultAnnouncement(filterValue, filteredCount) {
  const { t } = reactI18next.useTranslation("fx");
  const [resultAnnouncement, setResultAnnouncement] = React11.useState("");
  React11.useEffect(() => {
    if (!filterValue) {
...
function useMessageHeight(messageRef, valueStateMessage) {
  const [messageHeight, setMessageHeight] = React11.useState(0);
...
function usePopoverControls({
  ...
  const openPopover = React11.useCallback(() => {
  ...
  const closePopover = React11.useCallback(() => {
    if (controlledOpen === void 0) {
      setInternalOpen(false);
    }
    setFocusedIndex(-1);
  }, [controlledOpen, setInternalOpen, setFocusedIndex]);
  const handlePopoverClose = React11.useCallback(() => {
    if (controlledOpen === void 0) {
      setInternalOpen(false);
    }
    setFocusedIndex(-1);
  ...
  const togglePopover = React11.useCallback(() => {
    if (isOpen) {
      closePopover();
    } else {
      openPopover();
...
function Token({
  ...
  const tokenRef = React11.useRef(null);
  const { t } = reactI18next.useTranslation("fx");
  React11.useImperativeHandle(
    ref,
    () => ({
  ...
  const handleCloseKeyDown = React11.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        e.stopPropagation();
  ...
  const ariaDescription = readonly ? t("TOKEN_ARIA_LABEL") : `${t("TOKEN_ARIA_LABEL")}, ${t("TOKEN_ARIA_DELETABLE")}`;
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ref: tokenRef,
      ...
      className: cn(
        ...
        // State classes
        selected && !readonly ? "bg-sapphire-chrome-button-bg-selected border-sapphire-chrome-button-fg-selected text-foreground cursor-pointer" : readonly ? "bg-sapphire-canvas-primary border-border text-foreground cursor-default" : "bg-sapphire-background-tertiary border-border text-foreground hover:bg-sapphire-background-quaternary hover:border-sapphire-border-secondary cursor-pointer",
      ...
      children: [
        ...
        !readonly && /* @__PURE__ */ jsxRuntime.jsx(
          "span",
          {
            ...
            children: closeIcon || /* @__PURE__ */ jsxRuntime.jsx(DeclineIcon, { className: "h-3 w-3" })
          }
        )
      ]
    }
...
function useListItemSelection(options) {
  ...
  const shouldForwardTabAfter = React11.useCallback(() => {
  ...
  const shouldForwardTabBefore = React11.useCallback((target) => {
  ...
  const handleClick2 = React11.useCallback((e) => {
    if (disabled) return;
  ...
  const handleSelectionChange = React11.useCallback((checked) => {
    if (disabled) return;
...
function useListItemFocus(options) {
  ...
  const [isFocusedWithin, setIsFocusedWithin] = React11.useState(false);
  const [isActive, setIsActive] = React11.useState(false);
  const getItemIndex = React11.useCallback(() => {
  ...
  const handleFocus = React11.useCallback((e) => {
  ...
  const handleBlur = React11.useCallback((e) => {
  ...
  const handleMouseDownCapture = React11.useCallback(() => {
    setIsFocusedWithin(true);
  }, []);
  const focus = React11.useCallback(() => {
    itemRef.current?.focus();
  }, [itemRef]);
  const blur = React11.useCallback(() => {
...
function useListItemDragDrop(options) {
  ...
  const handleDragStart = React11.useCallback((e) => {
    if (!movable || disabled) {
      e.preventDefault();
      return;
    }
  ...
  const handleDragEnd = React11.useCallback((e) => {
  ...
  const handleDragEnter = React11.useCallback((e) => {
    e.preventDefault();
  }, []);
  const handleDragLeave = React11.useCallback((_e) => {
  }, []);
  return {
    isDragging,
    isDropTarget,
...
function useListItemRegistration(options) {
  ...
  const listContext = useOptionalListContext();
  const itemIndexRef = React11.useRef(-1);
  React11.useEffect(() => {
...
function CheckBox({
  ...
  const [internalChecked, setInternalChecked] = React11.useState(defaultChecked);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const isActive = isChecked || indeterminate;
  const doToggle = React11.useCallback(
    (originalEvent) => {
...
function RadioButton({
  ...
  const descriptionId = `${id}-description`;
  const [internalChecked, setInternalChecked] = React11.useState(defaultChecked);
  const [groupTabIndex, setGroupTabIndex] = React11.useState(null);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const setChecked = React11.useCallback(
    (checked) => {
      if (!isControlled) {
  ...
  const doSelect = React11.useCallback(
    (originalEvent) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "span",
    {
      ...
      children: [
        ...
        showDelete && (deleteButton ? React11__namespace.cloneElement(deleteButton, {
...
function ListItemBase(props) {
  ...
  const listContext = useOptionalListContext();
  const effectiveAccessibleRole = React11.useMemo(() => {
  ...
  const handleDeleteClick = React11.useCallback(() => {
    if (itemKey) {
...
function ListItem(props) {
  ...
  const handleDetailButtonClick = React11__namespace.useCallback((detail) => {
...
var flattenChildren = (nodes) => {
  const result = [];
  React11.Children.forEach(nodes, (child) => {
    if (!React11.isValidElement(child)) return;
    if (child.type === React11__namespace.default.Fragment) {
...
function Tokenizer({
  ...
  const nMoreRef = React11.useRef(null);
  const selectionAnchorRef = React11.useRef(-1);
  const skipNextFocusRef = React11.useRef(false);
  const [focusedIndex, setFocusedIndex] = React11.useState(-1);
  const [selectedIndices, setSelectedIndices] = React11.useState(/* @__PURE__ */ new Set());
  const [visibleCount, setVisibleCount] = React11.useState(null);
  const [internalExpanded] = React11.useState(false);
  const [popoverOpen, setPopoverOpen] = React11.useState(false);
  const [liveAnnouncement, setLiveAnnouncement] = React11.useState("");
  const { t } = reactI18next.useTranslation("fx");
  ...
  const tokenWidthsRef = React11.useRef([]);
  const needsMeasureRef = React11.useRef(true);
  const zeroWidthRetriesRef = React11.useRef(0);
  React11.useEffect(() => {
    needsMeasureRef.current = true;
    zeroWidthRetriesRef.current = 0;
    setVisibleCount(null);
  }, [tokenCount]);
  const measureAndRecalculate = React11.useCallback(() => {
  ...
  const lastParentWidthRef = React11.useRef(0);
  React11.useEffect(() => {
  ...
  const handleNMoreClick = React11.useCallback(() => {
    if (onNMoreClick) {
      onNMoreClick();
    } else {
  ...
  const handleTokenSelect = React11.useCallback(
    (index, detail) => {
      if (disabled) return;
  ...
  const handleTokenDelete = React11.useCallback(
    (index, _detail) => {
      if (disabled) return;
  ...
  const handlePopoverTokenDelete = React11.useCallback(
    (originalIndex) => {
      if (disabled || readonly) return;
  ...
  const handleClearAll = React11.useCallback(() => {
    if (disabled || readonly) return;
  ...
  const scrollTokenIntoView = React11.useCallback((index) => {
  ...
  const visibleTokens = tokens.slice(0, effectiveVisible).map((token, index) => {
    return React11.cloneElement(token, {
      key: token.key ?? index,
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              visibleTokens,
              expanded && showClearAll && tokenCount > 0 && !readonly && !disabled && /* @__PURE__ */ jsxRuntime.jsx(
                "span",
                {
                  role: "button",
                  tabIndex: 0,
        ...
        showNMore && /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
          /* @__PURE__ */ jsxRuntime.jsx(
            "span",
            {
              ref: nMoreRef,
          ...
          !onNMoreClick && /* @__PURE__ */ jsxRuntime.jsx(
            ResponsivePopover,
            {
              opener: containerRef,
              open: popoverOpen,
                ...
                List,
                {
                  ...
                  renderItem: (item) => /* @__PURE__ */ jsxRuntime.jsx(
                    ListItem,
                    {
                      ...
                      deleteButton: !readonly ? /* @__PURE__ */ jsxRuntime.jsx(
                        Button,
                        {
                          design: "Tertiary",
                          size: "Small",
                          iconOnly: true,
                          icon: /* @__PURE__ */ jsxRuntime.jsx(DeclineIcon, { className: "h-3 w-3" }),
...
function MultiComboBox({
  ...
  const containerRef = React11.useRef(null);
  const inputRef = React11.useRef(null);
  const tokenizerRef = React11.useRef(null);
  const listRef = React11.useRef(null);
  const selectAllRef = React11.useRef(null);
  const selectAllFocusedRef = React11.useRef(false);
  const messageRef = React11.useRef(null);
  const [portalTarget, setPortalTarget] = React11.useState(null);
  ...
  const [filterValue, setFilterValue] = React11.useState("");
  const [focusedIndex, setFocusedIndex] = React11.useState(-1);
  const [selectedOnTop, setSelectedOnTop] = React11.useState(false);
  const [inputFocused, setInputFocused] = React11.useState(false);
  const [internalSelectedValues, setInternalSelectedValues] = React11.useState(
    () => new Set(defaultSelectedValues)
  );
  const inputValue = controlledValue ?? internalValue;
  const isOpen = controlledOpen ?? internalOpen;
  const selectedValuesSet = React11.useMemo(() => {
  ...
  const baseId = React11.useMemo(() => `multicombobox-${Math.random().toString(36).slice(2, 9)}`, []);
  const popoverId = `${baseId}-popover`;
  const inputId = `${baseId}-input`;
  const messageId = `${baseId}-message`;
  const accessibleDescSpanId = `${baseId}-accessible-desc`;
  const flattenedItems = React11.useMemo(() => {
    const items = [];
    React11.Children.forEach(children, (child, groupIdx) => {
      if (!React11.isValidElement(child)) return;
      if (child.type?.displayName === "MultiComboBoxItemGroup") {
        const groupProps = child.props;
        React11.Children.forEach(groupProps.children, (groupChild) => {
          if (React11.isValidElement(groupChild)) {
            items.push({
              element: groupChild,
              groupIndex: groupIdx
            });
  ...
  const navigableItems = React11.useMemo(() => {
  ...
  const selectedItems = React11.useMemo(() => {
    const items = [];
  ...
  const filteredCount = React11.useMemo(
  ...
  const buildSelectedItemsList = React11.useCallback(
    (valuesSet) => {
      const items = [];
      for (const { element } of flattenedItems) {
  ...
  const toggleItem = React11.useCallback(
    (itemElement) => {
  ...
  const handleSelectAll = React11.useCallback(() => {
  ...
  const selectAllState = React11.useMemo(() => {
  ...
  const renderChildren = React11.useCallback(() => {
    let itemIndex = 0;
    ...
    if (selectedOnTop) {
      ...
      React11.Children.forEach(children, (child) => {
        ...
        if (child.type?.displayName === "MultiComboBoxItemGroup") {
          ...
          React11.Children.forEach(groupProps.children, (groupChild) => {
            if (!React11.isValidElement(groupChild)) return;
            const props = getItemProps(groupChild);
            if (props.selected) {
              selectedGroupChildren.push(
                React11.cloneElement(groupChild, props)
              );
            }
          });
          ...
          if (props.selected) {
            result.push(
              React11.cloneElement(child, props)
            );
          }
        }
      });
    ...
    return React11.Children.map(children, (child) => {
      ...
      if (child.type?.displayName === "MultiComboBoxItemGroup") {
        ...
        const groupChildren = React11.Children.map(groupProps.children, (groupChild) => {
          if (!React11.isValidElement(groupChild)) return groupChild;
          const props = getItemProps(groupChild);
          if (props.isVisible) hasVisibleChild = true;
          return React11.cloneElement(groupChild, props);
        });
  ...
  const handleListKeyDown = React11.useCallback(
    (e) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { ref, className: cn("relative w-full", className), children: [
    /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
        ...
        children: [
          icon && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex items-center justify-center shrink-0 text-sapphire-text-tertiary", children: icon }),
          selectedItems.length > 0 && /* @__PURE__ */ jsxRuntime.jsx(
            Tokenizer,
            {
              ...
              children: selectedItems.map((item, idx) => /* @__PURE__ */ jsxRuntime.jsx(
                Token,
                {
                  text: item.text,
                  readonly,
          ...
          showClear && /* @__PURE__ */ jsxRuntime.jsx(ClearButton, { onClick: handleClear, accessibleName: "Clear selection", comboBoxSize: size }),
          !readonly && /* @__PURE__ */ jsxRuntime.jsx(
            DropdownArrowButton,
            {
              onClick: () => {
    ...
    name && selectedItems.map((item, i) => /* @__PURE__ */ jsxRuntime.jsx("input", { type: "hidden", name, value: item.value }, i)),
    /* @__PURE__ */ jsxRuntime.jsx(
      ComboBoxPopover,
      {
        open: isOpen,
              ...
              children: [
                /* @__PURE__ */ jsxRuntime.jsxs(
                  "span",
                  {
                    ...
                    "aria-hidden": "true",
                    children: [
                      selectAllState === "checked" && /* @__PURE__ */ jsxRuntime.jsx(AcceptIcon, { className: "size-3" }),
                      selectAllState === "indeterminate" && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "block w-2 h-0.5 bg-white rounded-sm" })
                    ]
                  }
                ),
                /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex-1 font-medium", children: t("MULTICOMBOBOX_SELECT_ALL") })
              ]
            }
          )
        ) : void 0,
        children: /* @__PURE__ */ jsxRuntime.jsx("div", { ref: listRef, onKeyDown: handleListKeyDown, children: renderChildren() })
      }
    ),
    valueStateMessage && /* @__PURE__ */ jsxRuntime.jsx(
      ValueStateMessage,
      {
        isOpen,
        hasValueState,
    ...
    accessibleDescription && /* @__PURE__ */ jsxRuntime.jsx("span", { id: accessibleDescSpanId, className: "sr-only", children: accessibleDescription }),
    /* @__PURE__ */ jsxRuntime.jsx(LiveRegion, { message: resultAnnouncement })
  ] });
}
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx(
          "span",
          {
            ...
            "aria-hidden": "true",
            children: selected && /* @__PURE__ */ jsxRuntime.jsx(AcceptIcon, { className: "size-3" })
          }
        ),
        /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex-1 truncate", children: children || displayText }),
        additionalText && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "shrink-0 text-sapphire-text-tertiary text-sm", children: additionalText })
      ]
    }
  );
}
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("div", { role: "presentation", children })
      ]
    }
  );
}
...
exports.MultiComboBoxItemGroup = MultiComboBoxItemGroup;
