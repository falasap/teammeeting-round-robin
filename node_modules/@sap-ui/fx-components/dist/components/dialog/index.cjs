...
function Icon({
  ...
  const iconRef = React10.useRef(null);
  ...
  const handleClick2 = React10.useCallback(
    (e) => {
  ...
  if (!pathData7 || pathData7.length === 0) {
    return null;
  }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("g", { role: "presentation", children: pathData7.map((path, index) => /* @__PURE__ */ jsxRuntime.jsx("path", { d: path }, index)) })
      ]
    }
  );
}
...
var MessageErrorIcon = Object.assign(
  React10__namespace.default.forwardRef(
...
function BusyIndicator({
  ...
  const [isBusy, setIsBusy] = React10.useState(false);
  const labelId = React10.useId();
  const hasChildren = React10__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React10.useRef(null);
  const [animationPhase, setAnimationPhase] = React10.useState("idle");
  const [displayedState, setDisplayedState] = React10.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React10.useState(void 0);
  ...
  const handleKeyUp = React10.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function useIsPhone() {
  const [phone, setPhone] = React10__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React10__namespace.useState(false);
  const dialogId = React10__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React10__namespace.useRef(null);
  const headerRef = React10__namespace.useRef(null);
  const contentRef = React10__namespace.useRef(null);
  const previousActiveElementRef = React10__namespace.useRef(null);
  const returnValueRef = React10__namespace.useRef(void 0);
  const isClosingRef = React10__namespace.useRef(false);
  const didPopRef = React10__namespace.useRef(false);
  const [isDragging, setIsDragging] = React10__namespace.useState(false);
  const [position, setPosition] = React10__namespace.useState(null);
  const positionRef = React10__namespace.useRef(position);
  React10__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React10__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React10__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React10__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React10__namespace.useCallback(() => {
  ...
  const clampPosition = React10__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React10__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React10__namespace.useRef(isPopoverActuallyOpen);
  React10__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React10__namespace.useRef(onAfterClose);
  React10__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React10__namespace.useRef(handleClose);
  React10__namespace.useEffect(() => {
  ...
  const handleMouseDown = React10__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React10__namespace.useCallback(
    (e) => {
  ...
  const handleResizeMouseDown = React10__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
