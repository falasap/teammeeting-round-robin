...
var getCachedLocaleDataInstance = require('@ui5/webcomponents-localization/dist/getCachedLocaleDataInstance.js');
var Locale2 = require('@ui5/webcomponents-base/dist/locale/Locale.js');
require('@ui5/webcomponents-localization/dist/Assets.js');
require('@ui5/webcomponents-localization/dist/sap/ui/core/date/Gregorian.js');
var LocaleData_js = require('@ui5/webcomponents-base/dist/asset-registries/LocaleData.js');
var reactI18next = require('react-i18next');
var DateFormat2 = require('@ui5/webcomponents-localization/dist/DateFormat.js');
require('chrono-node');
...
var getCachedLocaleDataInstance__default = /*#__PURE__*/_interopDefault(getCachedLocaleDataInstance);
var Locale2__default = /*#__PURE__*/_interopDefault(Locale2);
var DateFormat2__default = /*#__PURE__*/_interopDefault(DateFormat2);
...
function Icon({
  ...
  const handleClick2 = React19.useCallback(
    (e) => {
  ...
  if (!pathData12 || pathData12.length === 0) {
    return null;
  }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("g", { role: "presentation", children: pathData12.map((path, index) => /* @__PURE__ */ jsxRuntime.jsx("path", { d: path }, index)) })
      ]
    }
  );
}
...
var CalendarIcon = Object.assign(
  React19__namespace.default.forwardRef(
...
function getLocale(localeCode) {
  return new Locale2__default.default(localeCode || "en-US");
}
function getWeekStartDay(localeCode) {
  const localeData = getCachedLocaleDataInstance__default.default(getLocale(localeCode));
  return localeData.getFirstDayOfWeek();
}
function getLocaleDateFormat(localeCode) {
  const localeData = getCachedLocaleDataInstance__default.default(getLocale(localeCode));
...
function getWeekEnd(date, weekStartsOn = 1) {
...
function getMonthDays(date, weekStartsOn = 1) {
...
function westernWeekNumber(date) {
  const yearStart = new Date(date.getFullYear(), 0, 1);
  const startDow = yearStart.getDay();
  const dayOfYear = Math.floor((date.getTime() - yearStart.getTime()) / 864e5);
  return Math.floor((dayOfYear + startDow) / 7) + 1;
}
function middleEasternWeekNumber(date) {
  const yearStart = new Date(date.getFullYear(), 0, 1);
  const startDow = (yearStart.getDay() + 1) % 7;
  const dayOfYear = Math.floor((date.getTime() - yearStart.getTime()) / 864e5);
  return Math.floor((dayOfYear + startDow) / 7) + 1;
}
function getWeekNumber(date, numbering = "ISO_8601" /* ISO_8601 */) {
  switch (numbering) {
    case "WesternTraditional" /* WesternTraditional */:
...
function isSameDayUtil(date1, date2) {
  return date1.getFullYear() === date2.getFullYear() && date1.getMonth() === date2.getMonth() && date1.getDate() === date2.getDate();
}
function isSameMonthUtil(date, compareDate) {
...
function isDateAfter(date, compareDate) {
  return date.getTime() > compareDate.getTime();
}
...
function getMonthName(date, formatStr = "long", localeCode) {
  const localeData = getCachedLocaleDataInstance__default.default(getLocale(localeCode));
...
function useCalendarSelection(mode, initialDates, rangeStart, rangeEnd, minDate, maxDate, disabledDates) {
  const [selectedDates, setSelectedDates] = React19.useState(() => {
    if (mode === "Range") return [];
    return initialDates?.map((d) => parseDate(d)) || [];
  });
  const [rangeStartDate, setRangeStartDate] = React19.useState(
    () => rangeStart ? parseDate(rangeStart) : null
  );
  const [rangeEndDate, setRangeEndDate] = React19.useState(
    () => rangeEnd ? parseDate(rangeEnd) : null
  );
  const [hoveredDate, setHoveredDate] = React19.useState(null);
  const minDateObj = minDate ? parseDate(minDate) : null;
  const maxDateObj = maxDate ? parseDate(maxDate) : null;
  const isDateDisabled = React19.useCallback(
    (date) => {
  ...
  const isDateSelected = React19.useCallback(
    (date) => {
      if (mode === "Range") {
  ...
  const isRangeStart = React19.useCallback(
    (date) => {
  ...
  const isRangeEnd = React19.useCallback(
    (date) => {
  ...
  const isDateBetweenRange = React19.useCallback(
    (date) => {
      if (mode !== "Range" || !rangeStartDate) return false;
      if (rangeEndDate) {
        return isDateInRange(date, rangeStartDate, rangeEndDate) && !isSameDayUtil(date, rangeStartDate) && !isSameDayUtil(date, rangeEndDate);
      }
      if (hoveredDate) {
        ...
        return isDateInRange(date, previewStart, previewEnd) && !isSameDayUtil(date, previewStart) && !isSameDayUtil(date, previewEnd);
      }
      return false;
    },
    [mode, rangeStartDate, rangeEndDate, hoveredDate]
  );
  const selectDate = React19.useCallback(
    (date) => {
      if (isDateDisabled(date)) return null;
      let newSelectedValues = [];
      let newSelectedTimestamps = [];
      if (mode === "Single") {
        setSelectedDates([date]);
        newSelectedValues = [formatDateToISO(date)];
        newSelectedTimestamps = [Math.floor(date.getTime() / 1e3)];
      } else if (mode === "Multiple") {
        let newDates = [];
        setSelectedDates((prev) => {
          const exists2 = prev.some((d) => isSameDayUtil(d, date));
          if (exists2) {
        ...
        newSelectedTimestamps = newDates.map((d) => Math.floor(d.getTime() / 1e3));
      } else if (mode === "Range") {
        if (!rangeStartDate || rangeStartDate && rangeEndDate) {
          ...
          newSelectedTimestamps = [
            Math.floor(newStart.getTime() / 1e3),
            Math.floor(newEnd.getTime() / 1e3)
          ];
        }
      }
  ...
  const getSelectedValuesAsISO = React19.useCallback(() => {
    if (mode === "Range") {
      const values = [];
  ...
  const getSelectedTimestamps = React19.useCallback(() => {
    if (mode === "Range") {
      const timestamps = [];
      if (rangeStartDate) timestamps.push(Math.floor(rangeStartDate.getTime() / 1e3));
      if (rangeEndDate) timestamps.push(Math.floor(rangeEndDate.getTime() / 1e3));
      return timestamps;
    }
    return selectedDates.map((d) => Math.floor(d.getTime() / 1e3));
  }, [mode, selectedDates, rangeStartDate, rangeEndDate]);
...
function useSpecialDates(specialDates) {
  const getSpecialDate = React19.useCallback(
    (date) => {
...
function BusyIndicator({
  ...
  const overlayRef = React19.useRef(null);
  const [isBusy, setIsBusy] = React19.useState(false);
  const labelId = React19.useId();
  const hasChildren = React19__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React19.useRef(null);
  const [animationPhase, setAnimationPhase] = React19.useState("idle");
  const [displayedState, setDisplayedState] = React19.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React19.useState(void 0);
  ...
  const handleKeyUp = React19.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx(
          Button,
          {
            ...
            size: "Medium" /* Medium */,
            iconOnly: true,
            icon: /* @__PURE__ */ jsxRuntime.jsx(SlimArrowLeftIcon, { className: "h-4 w-4 rtl:rotate-180" }),
            accessibleName: previousTooltip,
            tooltip: previousTooltip,
            disabled: isPreviousDisabled,
            onClick: onPrevious
          }
        ),
        /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex items-center gap-2 flex-1 justify-center", role: "group", "aria-label": "Current date", children: [
          monthText && /* @__PURE__ */ jsxRuntime.jsx(
            Button,
            {
              design: "Tertiary" /* Tertiary */,
              size: "Medium" /* Medium */,
              accessibleName: `${monthText}, Select month`,
              onClick: onMonthClick,
              className: "text-sapphire-text-accent",
              children: cldrReady ? monthText : /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-4 w-16 rounded bg-muted animate-pulse" })
            }
          ),
          /* @__PURE__ */ jsxRuntime.jsx(
            Button,
            {
              design: "Tertiary" /* Tertiary */,
              size: "Medium" /* Medium */,
              accessibleName: `${yearText}, Select year`,
              onClick: onYearClick,
              className: "text-sapphire-text-accent",
              children: cldrReady ? yearText : /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-4 w-10 rounded bg-muted animate-pulse" })
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntime.jsx(
          Button,
          {
            ...
            size: "Medium" /* Medium */,
            iconOnly: true,
            icon: /* @__PURE__ */ jsxRuntime.jsx(SlimArrowRightIcon, { className: "h-4 w-4 rtl:rotate-180" }),
            accessibleName: nextTooltip,
            tooltip: nextTooltip,
...
function ensureCldr(localeStr) {
  let p = cache.get(localeStr);
  if (!p) {
    const loc = new Locale2__default.default(localeStr);
    p = LocaleData_js.fetchCldr(loc.getLanguage(), loc.getRegion(), loc.getScript()).catch(
      (err) => {
        console.warn(
          `[fx-components] CLDR load failed for "${localeStr}":`,
          err
        );
        cache.delete(localeStr);
      }
    );
...
function isCldrLoaded(localeStr) {
  try {
    const localeData = getCachedLocaleDataInstance__default.default(new Locale2__default.default(localeStr));
...
// src/i18n/useEnsureCldr.ts
function useEnsureCldr(locale = "en-US") {
  const [readyLocale, setReadyLocale] = React19.useState(
...
var dayCellInnerVariants = cva(
  "w-6 h-6 rounded flex items-center justify-center transition-colors",
  {
    variants: {
      variant: {
        default: "hover:bg-sapphire-bg-secondary",
        selected: "bg-sapphire-brand-hover-background text-sapphire-text-on-surface hover:bg-sapphire-brand-hover-background hover:text-sapphire-text-on-surface",
        ...
        rangeBetween: "border border-sapphire-border-accent bg-sapphire-brand-toggle-background text-sapphire-text hover:bg-sapphire-brand-toggle-background",
...
var FALLBACK_WIDE_DAYS = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
var FALLBACK_FIRST_DAY_OF_WEEK = 1;
var FALLBACK_WIDE_MONTHS = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
function DayPicker({
  currentMonth,
  focusedDate,
  autoFocus = false,
  ...
  const effectiveFirstDay = React19__namespace.default.useMemo(() => {
  ...
  const weekDays = React19__namespace.default.useMemo(() => {
    if (!cldrReady) return null;
    const localeData = getCachedLocaleDataInstance__default.default(getLocale(locale));
  ...
  const { wideDays, wideMonths } = React19__namespace.default.useMemo(() => {
    if (!cldrReady) return {
  ...
  const focusedCellRef = React19.useRef(null);
  const isMountedRef = React19.useRef(false);
  const setFocusedCellRef = React19.useCallback((node) => {
  ...
  const handleMouseOver = React19.useCallback(
    (date) => {
      if (isRangeSelectionInProgress && onHoverDate) {
        onHoverDate(date);
      }
  ...
  const handleMouseLeave = React19.useCallback(() => {
    if (isRangeSelectionInProgress && onHoverDate) {
      onHoverDate(null);
    }
  }, [isRangeSelectionInProgress, onHoverDate]);
  const getDayCellVariant = (date) => {
  ...
  const getDayCellSpecialType = (date) => {
    const special = getSpecialDate(date);
    return special?.type || "None" /* None */;
  };
  ...
  const getRangeStripClass = (date, idx) => {
    const isStart = isRangeStart(date);
    const isEnd = isRangeEnd(date);
  ...
  const gridCols = showWeekNumbers ? "grid-cols-[repeat(8,2rem)]" : "grid-cols-[repeat(7,2rem)]";
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { ref, className: cn("p-2 flex flex-col items-center", className), children: [
    /* @__PURE__ */ jsxRuntime.jsx("style", { children: `
          .range-strip {
            position: relative;
          }
    ...
    /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntime.jsxs("div", { role: "row", className: cn("grid", gridCols), children: [
            ...
            weekDays ? weekDays.map((day) => /* @__PURE__ */ jsxRuntime.jsx(
              "div",
              {
                role: "columnheader",
                className: "h-8 w-full flex items-center justify-center text-xs font-medium text-sapphire-text-tertiary",
              ...
              day
            )) : Array.from({ length: 7 }, (_, i) => /* @__PURE__ */ jsxRuntime.jsx(
              "div",
              {
                role: "columnheader",
                className: "h-8 w-full flex items-center justify-center",
                children: /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-3 w-6 rounded bg-muted animate-pulse" })
              },
              i
            ))
          ] }),
          Array.from({ length: Math.ceil(days.length / 7) }, (_, weekIndex) => {
            ...
            return /* @__PURE__ */ jsxRuntime.jsxs("div", { role: "row", className: cn("grid items-center", gridCols), children: [
              showWeekNumbers && /* @__PURE__ */ jsxRuntime.jsx(
                "div",
                {
                  role: "rowheader",
                  className: "h-8 w-full flex items-center justify-center text-xs text-sapphire-text-tertiary",
                  children: cldrReady ? weekNumbers[weekIndex] : /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-3 w-4 rounded bg-muted animate-pulse" })
                }
              ),
              weekDates.map((date, dayIndex) => {
                ...
                const gridcell = /* @__PURE__ */ jsxRuntime.jsx(
                  "div",
                  {
                    ref: isFocused ? setFocusedCellRef : void 0,
                    role: "gridcell",
                    "aria-label": ariaLabel,
                    ...
                    children: /* @__PURE__ */ jsxRuntime.jsx("span", { className: dayCellInnerVariants({ variant }), children: cldrReady ? format(date, "d") : /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-3 w-4 rounded bg-muted animate-pulse" }) })
                  }
                );
                return rangeStripClass ? /* @__PURE__ */ jsxRuntime.jsx("div", { className: rangeStripClass, children: gridcell }, formatDateToISO(date)) : /* @__PURE__ */ jsxRuntime.jsx(React19__namespace.default.Fragment, { children: gridcell }, formatDateToISO(date));
              })
            ] }, weekKey);
          })
        ]
...
var monthCellVariants = cva(
  "min-h-[2.75rem] w-full rounded-lg text-sm font-normal transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
...
function MonthPicker({
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { ref, className: cn("p-3 w-full", className), children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex flex-col gap-2 w-full", role: "grid", "aria-label": "Select month", children: Array.from({ length: 4 }, (_, rowIndex) => {
    const rowMonths = months.slice(rowIndex * 3, (rowIndex + 1) * 3);
    return /* @__PURE__ */ jsxRuntime.jsx("div", { role: "row", className: "grid grid-cols-3 gap-2", children: rowMonths.map((month) => {
      const date = createDate(currentYear, month);
      const monthName = cldrReady ? getMonthName(date, "short", locale) : "";
      const isFocused = month === focusedMonth;
      const isSelected = month === selectedMonth;
      return /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          ref: isFocused ? focusedCellRef : void 0,
          role: "gridcell",
          ...
          children: cldrReady ? monthName : /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-3.5 w-8 rounded bg-muted animate-pulse" })
        },
        month
      );
    }) }, rowIndex);
...
function YearPicker({
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("div", { ref, className: cn("p-3 w-full", className), children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex flex-col gap-2 w-full", role: "grid", "aria-label": "Select year", children: Array.from({ length: 5 }, (_, rowIndex) => {
    const rowYears = years.slice(rowIndex * 4, (rowIndex + 1) * 4);
    return /* @__PURE__ */ jsxRuntime.jsx("div", { role: "row", className: "grid grid-cols-4 gap-2", children: rowYears.map((year) => {
...
var legendItemBoxVariants = cva(
  "h-4 w-4 border border-sapphire-border flex items-center justify-center rounded-sm",
  {
    variants: {
      type: {
        ["None" /* None */]: "",
        ["Today" /* Today */]: "border-2 border-sapphire-border-accent bg-transparent",
        ["Selected" /* Selected */]: "bg-sapphire-brand-hover-background",
        ["Working" /* Working */]: "bg-sapphire-positive-bg",
        ["NonWorking" /* NonWorking */]: "bg-sapphire-negative-bg",
        ["Type01" /* Type01 */]: "bg-blue-500",
        ["Type02" /* Type02 */]: "bg-green-500",
        ["Type03" /* Type03 */]: "bg-yellow-500",
        ["Type04" /* Type04 */]: "bg-red-500",
        ["Type05" /* Type05 */]: "bg-purple-500",
        ["Type06" /* Type06 */]: "bg-pink-500",
        ["Type07" /* Type07 */]: "bg-indigo-500",
        ["Type08" /* Type08 */]: "bg-cyan-500",
        ["Type09" /* Type09 */]: "bg-teal-500",
        ["Type10" /* Type10 */]: "bg-orange-500",
        ["Type11" /* Type11 */]: "bg-lime-500",
        ["Type12" /* Type12 */]: "bg-emerald-500",
        ["Type13" /* Type13 */]: "bg-sky-500",
        ["Type14" /* Type14 */]: "bg-violet-500",
        ["Type15" /* Type15 */]: "bg-fuchsia-500",
        ["Type16" /* Type16 */]: "bg-rose-500",
        ["Type17" /* Type17 */]: "bg-amber-500",
        ["Type18" /* Type18 */]: "bg-slate-500",
        ["Type19" /* Type19 */]: "bg-zinc-500",
        ["Type20" /* Type20 */]: "bg-neutral-500"
      }
    },
    defaultVariants: {
      type: "None" /* None */
...
function CalendarLegendItem({ type, text, onClick, className, ref }) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: cn(legendItemBoxVariants({ type })), children: isSelected && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "h-1.5 w-1.5 rounded-full bg-sapphire-text-on-surface" }) }),
        /* @__PURE__ */ jsxRuntime.jsx("span", { className: "text-sm text-sapphire-text truncate", children: text })
      ]
    }
  );
}
var getDefaultLegendItemText = (type, _locale) => {
  const labels = {
    ["Today" /* Today */]: "Today",
...
function CalendarLegend({
  ...
  const defaultItems = [
    {
      type: "Today" /* Today */,
      text: getDefaultLegendItemText("Today" /* Today */),
      hidden: hideToday
    },
    {
      type: "Selected" /* Selected */,
      text: getDefaultLegendItemText("Selected" /* Selected */),
      hidden: hideSelectedDay
    },
    {
      type: "Working" /* Working */,
      text: getDefaultLegendItemText("Working" /* Working */),
      hidden: hideWorkingDay
    },
    {
      type: "NonWorking" /* NonWorking */,
      text: getDefaultLegendItemText("NonWorking" /* NonWorking */),
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      ...
      children: allItems.map((item, index) => /* @__PURE__ */ jsxRuntime.jsx(
        CalendarLegendItem,
        {
          type: item.type,
          text: item.text,
          onClick: onLegendItemSelect ? () => onLegendItemSelect({ type: item.type, text: item.text }) : void 0
        },
        `${item.type}-${index}`
      ))
    }
...
function getInitialView(pickersMode) {
  switch (pickersMode) {
    case "YEAR" /* YEAR */:
      return "year" /* Year */;
...
function Calendar({
  ...
  pickersMode = "DAY_MONTH_YEAR" /* DAY_MONTH_YEAR */,
  minDate,
  maxDate,
  selectedDates,
  rangeStartDate,
  ...
  const { i18n } = reactI18next.useTranslation("fx");
  const locale = localeProp || i18n.language;
  const cldrReady = useEnsureCldr(locale);
  const [currentView, setCurrentView] = React19.useState(
    () => getInitialView(pickersMode)
  );
  const [displayedDate, setDisplayedDate] = React19.useState(() => {
  ...
  const [focusDate, setFocusDate] = React19.useState(displayedDate);
  const [yearRangeStart, setYearRangeStart] = React19.useState(() => {
    const year = getYear(displayedDate);
    return Math.floor(year / 20) * 20;
  });
  const minDateObj = React19.useMemo(() => minDate ? parseDate(minDate) : void 0, [minDate]);
  const maxDateObj = React19.useMemo(() => maxDate ? parseDate(maxDate) : void 0, [maxDate]);
  const isPreviousDisabled = React19.useMemo(() => {
    if (!minDateObj) return false;
    if (currentView === "day" /* Day */) {
  ...
  const handleMonthSelect = React19.useCallback((month) => {
    const newDate = createDate(getYear(displayedDate), month);
    setDisplayedDate(newDate);
    setFocusDate(newDate);
    if (pickersMode === "MONTH_YEAR" /* MONTH_YEAR */) {
      if (onSelectionChange) {
        const isoValue = `${getYear(newDate)}-${String(month + 1).padStart(2, "0")}`;
        onSelectionChange({
          selectedValues: [isoValue],
          selectedDates: [Math.floor(newDate.getTime() / 1e3)],
          timestamp: Math.floor(newDate.getTime() / 1e3)
        });
      }
    } else {
      setCurrentView("day" /* Day */);
    }
  }, [displayedDate, pickersMode, onSelectionChange]);
  const handleYearSelect = React19.useCallback((year) => {
    const newDate = createDate(year, getMonth(displayedDate));
    setDisplayedDate(newDate);
    setFocusDate(newDate);
    if (pickersMode === "YEAR" /* YEAR */) {
      if (onSelectionChange) {
        onSelectionChange({
  ...
  const handleShowMonthView = React19.useCallback(() => {
    if (pickersMode === "YEAR" /* YEAR */) return;
    setCurrentView("month" /* Month */);
    onShowMonthView?.();
  }, [onShowMonthView, pickersMode]);
  const handleShowYearView = React19.useCallback(() => {
    setCurrentView("year" /* Year */);
    onShowYearView?.();
  }, [onShowYearView]);
  const getHeaderText = React19.useCallback(() => {
    const year = getYear(displayedDate);
    const FALLBACK_LONG_MONTHS = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    const month = cldrReady ? getMonthName(displayedDate, "long", locale) : FALLBACK_LONG_MONTHS[displayedDate.getMonth()];
    if (currentView === "day" /* Day */) {
  ...
  const handleHoverDate = React19.useCallback((date) => {
    selection.setHoveredDate(date);
  }, [selection]);
  const handleFocusChange = React19.useCallback((date) => {
    setFocusDate(date);
    if (getMonth(date) !== getMonth(displayedDate)) {
      setDisplayedDate(date);
    }
  }, [displayedDate]);
  const handleContainerKeyDown = React19.useCallback((e) => {
    if (e.key === "F4") {
      e.preventDefault();
      if (e.shiftKey) {
        handleShowYearView();
  ...
  }, [handleShowMonthView, handleShowYearView]);
  const getTooltipText = React19.useCallback(() => {
    if (currentView === "day" /* Day */) {
      return { previous: "Previous month", next: "Next month" };
    } else if (currentView === "month" /* Month */) {
      return { previous: "Previous year", next: "Next year" };
    } else if (currentView === "year" /* Year */) {
      return { previous: "Previous year range", next: "Next year range" };
    }
    return { previous: "Previous", next: "Next" };
  }, [currentView]);
  const headerText = getHeaderText();
  const tooltipText = getTooltipText();
  const showMonthButton = pickersMode !== "YEAR" /* YEAR */ && pickersMode !== "MONTH_YEAR" /* MONTH_YEAR */ && currentView === "day" /* Day */;
  const rootRef = React19.useRef(null);
  React19.useImperativeHandle(ref, () => ({
    switchToDayPicker() {
      if (pickersMode === "YEAR" /* YEAR */ || pickersMode === "MONTH_YEAR" /* MONTH_YEAR */) return;
      setCurrentView("day" /* Day */);
    },
    switchToMonthPicker() {
      if (pickersMode === "YEAR" /* YEAR */) return;
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      className: cn(
        "flex flex-col rounded-lg border border-sapphire-border-primary bg-sapphire-card-bg-primary text-foreground shadow-[0_4px_10px_-2px_rgba(10,10,10,0.15),0_2px_5px_-2px_rgba(10,10,10,0.05)]",
        "w-fit min-w-[17rem]",
        className
      ),
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "relative w-full", children: [
          currentView === "day" /* Day */ && /* @__PURE__ */ jsxRuntime.jsx(
            DayPicker,
            {
          ...
          currentView === "month" /* Month */ && /* @__PURE__ */ jsxRuntime.jsx(
            MonthPicker,
            {
          ...
          currentView === "year" /* Year */ && (cldrReady ? /* @__PURE__ */ jsxRuntime.jsx(
            YearPicker,
            {
          ...
          ) : /* @__PURE__ */ jsxRuntime.jsx("div", { className: "p-3 w-full", children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex flex-col gap-2 w-full", children: Array.from({ length: 5 }, (_, row) => /* @__PURE__ */ jsxRuntime.jsx("div", { className: "grid grid-cols-4 gap-2", children: Array.from({ length: 4 }, (_2, col) => /* @__PURE__ */ jsxRuntime.jsx("div", { className: "min-h-[2.75rem] w-full flex items-center justify-center", children: /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-block h-3.5 w-8 rounded bg-muted animate-pulse" }) }, col)) }, row)) }) }))
        ] }),
...
function useIsRtl(ref) {
  const [isRtl, setIsRtl] = React19.useState(false);
  React19.useEffect(() => {
...
function Popover({
  ...
  const focusedBeforeOpen = React19.useRef(null);
  const [isOpen, setIsOpen] = React19.useState(false);
  const [pos, setPos] = React19.useState(null);
  const [resizeSize, setResizeSize] = React19.useState(null);
  const resizeState = React19.useRef(null);
  const [isDesktop2] = React19.useState(() => isDesktop());
  const containerRef = React19.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React19.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React19.useRef(() => {
  });
  const reposition = React19.useCallback(() => {
  ...
  const resizeHandleCorner = React19.useMemo(() => {
...
function useIsPhone() {
  const [phone, setPhone] = React19__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [internalOpen, setInternalOpen] = React19__namespace.useState(defaultOpen);
  const isOpen = controlledOpen ?? internalOpen;
  const onPhone = useIsPhone();
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React19__namespace.useState(false);
  const dialogId = React19__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React19__namespace.useRef(null);
  const headerRef = React19__namespace.useRef(null);
  const contentRef = React19__namespace.useRef(null);
  const previousActiveElementRef = React19__namespace.useRef(null);
  const returnValueRef = React19__namespace.useRef(void 0);
  const isClosingRef = React19__namespace.useRef(false);
  const didPopRef = React19__namespace.useRef(false);
  const [isDragging, setIsDragging] = React19__namespace.useState(false);
  const [position, setPosition] = React19__namespace.useState(null);
  const positionRef = React19__namespace.useRef(position);
  React19__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React19__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React19__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React19__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React19__namespace.useCallback(() => {
  ...
  const clampPosition = React19__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React19__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React19__namespace.useRef(isPopoverActuallyOpen);
  React19__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React19__namespace.useRef(onAfterClose);
  React19__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React19__namespace.useRef(handleClose);
  React19__namespace.useEffect(() => {
  ...
  const handleMouseDown = React19__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React19__namespace.useCallback(
    (e) => {
  ...
  const resizeState = React19__namespace.useRef(null);
  const handleResizeMouseDown = React19__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
  ...
  React19__namespace.useImperativeHandle(
    ref,
    () => ({
      show: () => {
        if (controlledOpen === void 0) {
...
function useIsPhone2() {
  const [phone] = React19.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const handleMobileClose = React19.useCallback(() => {
...
var TimeColumn = ({ values, selected, onSelect, label }) => {
  const containerRef = React19.useRef(null);
  const selectedRef = React19.useRef(null);
  React19.useEffect(() => {
  ...
  const handleKeyDown = React19.useCallback((e, idx) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex flex-col min-w-[3rem]", children: [
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "text-xs font-medium text-sapphire-text-tertiary text-center py-1 border-b border-sapphire-border", children: label }),
    /* @__PURE__ */ jsxRuntime.jsx(
      "div",
      {
        ref: containerRef,
...
function TimePicker({
  ...
  const hourValues = React19.useMemo(() => {
    if (is12h) {
  ...
  const handleMinuteChange = React19.useCallback((m) => {
    onTimeChange(hours, m, seconds);
  }, [hours, seconds, onTimeChange]);
  const handleSecondChange = React19.useCallback((s) => {
    onTimeChange(hours, minutes, s);
  }, [hours, minutes, onTimeChange]);
  const handlePeriodChange = React19.useCallback((p) => {
    const isPM = p === 1;
    let h24 = hours;
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "w-px bg-sapphire-border" }),
        /* @__PURE__ */ jsxRuntime.jsx(
          TimeColumn,
          {
            values: minuteValues,
        ...
        is12h && /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
          /* @__PURE__ */ jsxRuntime.jsx("div", { className: "w-px bg-sapphire-border" }),
          /* @__PURE__ */ jsxRuntime.jsx(
            TimeColumn,
            {
...
function DateTimePicker({
  ...
  const cldrReady = useEnsureCldr(locale);
  const isPhoneDevice = React19.useMemo(() => isPhone(), []);
  const [phoneTab, setPhoneTab] = React19.useState("date");
  const isMedium = size === "Medium" /* Medium */;
  const timeFormat = timeFormatProp || getLocaleTimeFormat(locale);
  const dateDisplayFormat = React19.useMemo(() => {
  ...
  const loc = React19.useMemo(() => new Locale2__default.default(locale), [locale]);
  const { displayFmt, valueFmt, isoDateFmt } = React19.useMemo(() => {
    if (!cldrReady) return { displayFmt: null, valueFmt: null, isoDateFmt: null };
    return {
      displayFmt: DateFormat2__default.default.getDateTimeInstance({ pattern: effectiveDisplayFormat }, loc),
      valueFmt: DateFormat2__default.default.getDateTimeInstance({ pattern: valueFormat }, loc),
      isoDateFmt: DateFormat2__default.default.getDateInstance({ pattern: "yyyy-MM-dd" }, loc)
    };
  }, [cldrReady, effectiveDisplayFormat, valueFormat, loc]);
  const formatDT = React19.useCallback((d) => displayFmt?.format(d) ?? "", [displayFmt]);
  const formatVal = React19.useCallback((d) => valueFmt?.format(d) ?? "", [valueFmt]);
  const formatISODate = React19.useCallback((d) => isoDateFmt?.format(d) ?? "", [isoDateFmt]);
  const parseDT = React19.useCallback((str, pattern) => {
    if (!cldrReady) return null;
    return DateFormat2__default.default.getDateTimeInstance({ pattern, strictParsing: true }, loc).parse(str) ?? null;
  }, [cldrReady, loc]);
  const parseISODate = React19.useCallback((str) => {
    if (!cldrReady) return null;
    return DateFormat2__default.default.getDateInstance({ pattern: "yyyy-MM-dd", strictParsing: true }, loc).parse(str) ?? null;
  }, [cldrReady, loc]);
  ...
  const [inputText, setInputText] = React19.useState("");
  const [isEditing, setIsEditing] = React19.useState(false);
  const [internalValueState, setInternalValueState] = React19.useState(valueState);
  const isOpen = controlledOpen ?? internalOpen;
  const value = controlledValue ?? internalValue;
  const selectedDate = cldrReady && value ? parseDT(value, valueFormat) ?? void 0 : void 0;
  const [tempHours, setTempHours] = React19.useState(0);
  const [tempMinutes, setTempMinutes] = React19.useState(0);
  const [tempSeconds, setTempSeconds] = React19.useState(0);
  const [tempDate, setTempDate] = React19.useState(void 0);
  const containerRef = React19.useRef(null);
  const inputRef = React19.useRef(null);
  const rootRef = React19.useRef(null);
  const skipFocusSyncRef = React19.useRef(false);
  const minDateObj = React19.useMemo(() => minDate ? parseISODate(minDate) : void 0, [minDate, parseISODate]);
  const maxDateObj = React19.useMemo(() => maxDate ? parseISODate(maxDate) : void 0, [maxDate, parseISODate]);
  const updateValueState = React19.useCallback((newState, valid) => {
  ...
  const buildDateTime = React19.useCallback((date, h, m, s) => {
    if (!date) return null;
    const dt = new Date(date);
    dt.setHours(h, m, s, 0);
    return dt;
  }, []);
  const commitDateTime = React19.useCallback((dt) => {
  ...
  const handleOk = React19.useCallback(() => {
  ...
  const handleCancel = React19.useCallback(() => {
    closePopover();
  }, [closePopover]);
  const handleCalendarSelect = React19.useCallback((detail) => {
  ...
  const handleTimeChange = React19.useCallback((h, m, s) => {
    setTempHours(h);
    setTempMinutes(m);
    setTempSeconds(s);
  }, []);
  const handleFocus = React19.useCallback(() => {
  ...
  const handleInputChange = React19.useCallback((e) => {
  ...
  const popoverFooter = /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex justify-end gap-2 p-3 border-t border-sapphire-border", children: [
    /* @__PURE__ */ jsxRuntime.jsx(Button, { design: "Tertiary", onClick: handleCancel, children: "Cancel" }),
    /* @__PURE__ */ jsxRuntime.jsx(Button, { design: "Primary", onClick: handleOk, children: "OK" })
  ] });
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { ref: rootRef, className: cn("relative", className), style, children: [
    /* @__PURE__ */ jsxRuntime.jsxs(
      "div",
      {
        ref: containerRef,
        ...
        children: [
          ...
          showClearIcon && displayValue && !disabled && !readonly && /* @__PURE__ */ jsxRuntime.jsx("span", { onMouseDown: (e) => e.preventDefault(), children: /* @__PURE__ */ jsxRuntime.jsx(
            Button,
            {
              design: "SecondaryNeutral" /* SecondaryNeutral */,
              size: isMedium ? "Small" /* Small */ : "Medium" /* Medium */,
              iconOnly: true,
              icon: /* @__PURE__ */ jsxRuntime.jsx(DeclineIcon, { className: isMedium ? "h-3 w-3" : "h-4 w-4" }),
              tabIndex: -1,
              onClick: handleClear,
          ...
          /* @__PURE__ */ jsxRuntime.jsx("span", { onMouseDown: (e) => e.preventDefault(), children: /* @__PURE__ */ jsxRuntime.jsx(
            Button,
            {
              design: "SecondaryNeutral" /* SecondaryNeutral */,
              size: isMedium ? "Small" /* Small */ : "Medium" /* Medium */,
              iconOnly: true,
              icon: /* @__PURE__ */ jsxRuntime.jsx(CalendarIcon, { className: isMedium ? "h-3 w-3" : "h-4 w-4" }),
              tabIndex: -1,
    ...
    (valueStateMessage || internalValueState === "Negative" /* Negative */) && /* @__PURE__ */ jsxRuntime.jsx(
      "div",
      {
    ...
    /* @__PURE__ */ jsxRuntime.jsx(
      ResponsivePopover,
      {
        ...
        children: isPhoneDevice ? (
          ...
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex flex-col w-full", children: [
            /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex border-b border-sapphire-border", children: [
              /* @__PURE__ */ jsxRuntime.jsxs(
                "button",
                {
                  type: "button",
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntime.jsx(CalendarIcon, { className: "h-4 w-4 inline-block mr-1" }),
                    "Date"
                  ]
                }
              ),
              /* @__PURE__ */ jsxRuntime.jsxs(
                "button",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntime.jsx(FobWatchIcon, { className: "h-4 w-4 inline-block mr-1" }),
                    "Time"
                  ]
                }
              )
            ] }),
            phoneTab === "date" ? /* @__PURE__ */ jsxRuntime.jsx(Calendar, { ...calendarProps }) : /* @__PURE__ */ jsxRuntime.jsx("div", { className: "p-4 flex justify-center", children: /* @__PURE__ */ jsxRuntime.jsx(TimePicker, { ...timePickerProps }) })
          ] })
        ) : (
          // Desktop mode: calendar + time picker side-by-side
          /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex", children: [
            /* @__PURE__ */ jsxRuntime.jsx(Calendar, { ...calendarProps, className: "border-0 shadow-none rounded-none" }),
            /* @__PURE__ */ jsxRuntime.jsx(TimePicker, { ...timePickerProps })
          ] })
        )
      }
    ),
    name && /* @__PURE__ */ jsxRuntime.jsx("input", { type: "hidden", name, value })
  ] });
}
