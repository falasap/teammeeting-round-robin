...
function useTabbarOverflow({
  ...
  const [visibleTabIds, setVisibleTabIds] = React12.useState(tabs.map((t) => t.id));
  const [overflowTabIds, setOverflowTabIds] = React12.useState([]);
  const [isReady, setIsReady] = React12.useState(false);
  const tabWidthsRef = React12.useRef(/* @__PURE__ */ new Map());
  const hasMeasuredRef = React12.useRef(false);
  const rafId = React12.useRef(null);
  const retryTimeoutRef = React12.useRef(null);
  const tabIds = React12.useMemo(() => tabs.map((t) => t.id), [tabs]);
  const measureTabs = React12.useCallback(() => {
  ...
  const calculateOverflow = React12.useCallback(() => {
  ...
  const scheduleCalculation = React12.useCallback(() => {
    if (rafId.current) {
...
function Icon({
  ...
  const handleClick2 = React12.useCallback(
    (e) => {
  ...
  if (!pathData13 || pathData13.length === 0) {
    return null;
  }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("g", { role: "presentation", children: pathData13.map((path, index) => /* @__PURE__ */ jsxRuntime.jsx("path", { d: path }, index)) })
      ]
    }
  );
}
...
var NavigationDownArrowIcon = Object.assign(
  React12__namespace.default.forwardRef(
...
function Popover({
  ...
  const focusedBeforeOpen = React12.useRef(null);
  const [isOpen, setIsOpen] = React12.useState(false);
  const [pos, setPos] = React12.useState(null);
  const [resizeSize, setResizeSize] = React12.useState(null);
  const resizeState = React12.useRef(null);
  const [isDesktop2] = React12.useState(() => isDesktop());
  const containerRef = React12.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React12.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React12.useRef(() => {
  });
  const reposition = React12.useCallback(() => {
  ...
  const focusLast = React12.useCallback(() => {
    if (!popoverRef.current) return;
    const el = getLastFocusable(popoverRef.current);
    (el || popoverRef.current).focus();
  }, []);
  const handleResizeMouseDown = React12.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React12.useMemo(() => {
...
function BusyIndicator({
  ...
  const [isBusy, setIsBusy] = React12.useState(false);
  const labelId = React12.useId();
  const hasChildren = React12__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React12.useRef(null);
  const [animationPhase, setAnimationPhase] = React12.useState("idle");
  const [displayedState, setDisplayedState] = React12.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React12.useState(void 0);
  ...
  const handleKeyUp = React12.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function useIsPhone() {
  const [phone, setPhone] = React12__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React12__namespace.useState(false);
  const dialogId = React12__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React12__namespace.useRef(null);
  const headerRef = React12__namespace.useRef(null);
  const contentRef = React12__namespace.useRef(null);
  const previousActiveElementRef = React12__namespace.useRef(null);
  const returnValueRef = React12__namespace.useRef(void 0);
  const isClosingRef = React12__namespace.useRef(false);
  const didPopRef = React12__namespace.useRef(false);
  const [isDragging, setIsDragging] = React12__namespace.useState(false);
  const [position, setPosition] = React12__namespace.useState(null);
  const positionRef = React12__namespace.useRef(position);
  React12__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React12__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React12__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React12__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React12__namespace.useCallback(() => {
  ...
  const clampPosition = React12__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React12__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React12__namespace.useRef(isPopoverActuallyOpen);
  React12__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React12__namespace.useRef(onAfterClose);
  React12__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React12__namespace.useRef(handleClose);
  React12__namespace.useEffect(() => {
  ...
  const handleMouseDown = React12__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React12__namespace.useCallback(
    (e) => {
  ...
  const handleMouseUp = React12__namespace.useCallback(() => {
    setIsDragging(false);
  }, []);
  const resizeState = React12__namespace.useRef(null);
  const handleResizeMouseDown = React12__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
...
function useIsPhone2() {
  const [phone] = React12.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const handleMobileClose = React12.useCallback(() => {
...
var ListContext = React12__namespace.createContext(null);
function useOptionalListContext() {
  return React12.useContext(ListContext);
}
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React12.useState(new Set(defaultKeys));
  const previousKeysRef = React12.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React12.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React12.useCallback(() => {
  ...
  const setSelected = React12.useCallback((key, selected) => {
  ...
  const toggleSelection = React12.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React12.useCallback((key) => {
    return selectedKeys.has(key);
  }, [selectedKeys]);
  const selectAll = React12.useCallback(() => {
  ...
  const deselectAll = React12.useCallback(() => {
    if (!isControlled) {
...
function useListNavigation({
  ...
  const [focusedIndex, setFocusedIndex] = React12.useState(-1);
  const getAllNavigable = React12.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React12.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const focusItem = React12.useCallback((index) => {
  ...
  const registerItem = React12.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListInner(props) {
  ...
  const growingButtonRef = React12.useRef(null);
  const beforeSentinelRef = React12.useRef(null);
  const afterSentinelRef = React12.useRef(null);
  const [showLoading, setShowLoading] = React12.useState(false);
  const [mediaRange, setMediaRange] = React12.useState("M");
  const itemRegistry = React12.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React12.useState(null);
  const [draggedKey, setDraggedKey] = React12.useState(null);
  const [dropTargetKey, setDropTargetKey] = React12.useState(null);
  const [dropPlacement, setDropPlacement] = React12.useState(null);
  ...
  const itemCount = React12.useMemo(() => {
    if (items) return items.length;
    return React12__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React12.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React12.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React12.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React12.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React12.useCallback((key, e) => {
  ...
  const handleDrop = React12.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React12.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React12.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React12.useCallback((e) => {
  ...
  const getAllTabbableElements = React12.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React12.useCallback(() => {
  ...
  const enterListAtFocusedItem = React12.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React12.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React12.useMemo(() => {
  ...
  const handleLoadMore = React12.useCallback(() => {
  ...
  const renderedItems = React12.useMemo(() => {
    ...
    if (items && renderItem) {
      return items.map((item, index) => {
        ...
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React12__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React12__namespace.Children.forEach(children, (child) => {
        if (React12__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React12__namespace.memo(ListInner);
...
function useListItemSelection(options) {
  ...
  const shouldForwardTabAfter = React12.useCallback(() => {
  ...
  const shouldForwardTabBefore = React12.useCallback((target) => {
  ...
  const handleClick2 = React12.useCallback((e) => {
    if (disabled) return;
  ...
  const handleSelectionChange = React12.useCallback((checked) => {
    if (disabled) return;
...
function useListItemFocus(options) {
  ...
  const [isFocusedWithin, setIsFocusedWithin] = React12.useState(false);
  const [isActive, setIsActive] = React12.useState(false);
  const getItemIndex = React12.useCallback(() => {
  ...
  const handleFocus = React12.useCallback((e) => {
  ...
  const handleBlur = React12.useCallback((e) => {
  ...
  const handleMouseDownCapture = React12.useCallback(() => {
    setIsFocusedWithin(true);
  }, []);
  const focus = React12.useCallback(() => {
    itemRef.current?.focus();
  }, [itemRef]);
  const blur = React12.useCallback(() => {
...
function useListItemDragDrop(options) {
  ...
  const handleDragStart = React12.useCallback((e) => {
    if (!movable || disabled) {
      e.preventDefault();
      return;
    }
  ...
  const handleDragEnd = React12.useCallback((e) => {
  ...
  const handleDragEnter = React12.useCallback((e) => {
    e.preventDefault();
  }, []);
  const handleDragLeave = React12.useCallback((_e) => {
  }, []);
  return {
    isDragging,
    isDropTarget,
...
function useListItemRegistration(options) {
  ...
  const listContext = useOptionalListContext();
  const itemIndexRef = React12.useRef(-1);
  React12.useEffect(() => {
...
function CheckBox({
  ...
  const [internalChecked, setInternalChecked] = React12.useState(defaultChecked);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const isActive = isChecked || indeterminate;
  const doToggle = React12.useCallback(
    (originalEvent) => {
...
function RadioButton({
  ...
  const descriptionId = `${id}-description`;
  const [internalChecked, setInternalChecked] = React12.useState(defaultChecked);
  const [groupTabIndex, setGroupTabIndex] = React12.useState(null);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const setChecked = React12.useCallback(
    (checked) => {
      if (!isControlled) {
  ...
  const doSelect = React12.useCallback(
    (originalEvent) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "span",
    {
      ...
      children: [
        ...
        showDelete && (deleteButton ? React12__namespace.cloneElement(deleteButton, {
...
function ListItemBase(props) {
  ...
  const listContext = useOptionalListContext();
  const effectiveAccessibleRole = React12.useMemo(() => {
  ...
  const handleDeleteClick = React12.useCallback(() => {
    if (itemKey) {
...
function ListItem(props) {
  ...
  const handleDetailButtonClick = React12__namespace.useCallback((detail) => {
...
function TabbarOverflowButton({ tabs, selectedTabId, onSelect, isFocused, onFocus, overflowId, onKeyDown, className, ref }) {
  const [isOpen, setIsOpen] = React12.useState(false);
  const buttonRef = React12.useRef(null);
  React12__namespace.default.useImperativeHandle(ref, () => buttonRef.current, []);
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "fx-tabbar-item", children: /* @__PURE__ */ jsxRuntime.jsxs(
      "button",
      {
        ref: buttonRef,
        type: "button",
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntime.jsx(NavigationDownArrowIcon, { className: "h-5 w-5 text-foreground" })
        ]
      }
    ) }),
    /* @__PURE__ */ jsxRuntime.jsx(
      ResponsivePopover,
      {
        ...
        accessibleRole: "None",
        children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "min-w-[200px] max-h-[300px] overflow-auto", children: /* @__PURE__ */ jsxRuntime.jsx(List, { selectionMode: "None", separators: "None", children: tabs.map((tab) => {
...
var Tabbar = ({
  ...
  const [internalSelectedTab, setInternalSelectedTab] = React12.useState(resolvedDefault);
  const [focusedId, setFocusedId] = React12.useState(tabItems[0]?.id ?? "");
  const tabListRef = React12.useRef(null);
  const tabRefs = React12.useRef(/* @__PURE__ */ new Map());
  const overflowButtonRef = React12.useRef(null);
  ...
  const handleTabSelect = React12.useCallback(
    (tab) => {
  ...
  const focusById = React12.useCallback((id) => {
    setFocusedId(id);
  ...
  const handleKeyUp = React12.useCallback((e) => {
    if (e.key === " ") {
      const items2 = [
        ...visibleTabs.map((t) => t.id),
        ...hasOverflow ? [OVERFLOW_BUTTON_ID] : []
  ...
  const renderTab = (tabId, shouldHide, posinset, setsize) => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsx("div", { className: "fx-tabbar-item", style: shouldHide ? { display: "none" } : void 0, children: /* @__PURE__ */ jsxRuntime.jsx(
      "button",
      {
        ref: (el) => {
          if (el) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: className || "relative flex flex-col h-12", children: [
    !noUnderline && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute bottom-0 left-0 right-0 h-px bg-border" }),
    /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex flex-1 min-h-0 min-w-0 w-full items-stretch", children: [
      /* @__PURE__ */ jsxRuntime.jsx(
        "div",
        {
          ref: tabListRef,
      ...
      enableOverflow && isReady && hasOverflow && /* @__PURE__ */ jsxRuntime.jsx(
...
exports.TabbarOverflowButton = TabbarOverflowButton;
exports.useTabbarOverflow = useTabbarOverflow;
