...
function Icon({
  ...
  const handleClick2 = React15.useCallback(
    (e) => {
  ...
  if (!pathData16 || pathData16.length === 0) {
    return null;
  }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("g", { role: "presentation", children: pathData16.map((path, index) => /* @__PURE__ */ jsxRuntime.jsx("path", { d: path }, index)) })
      ]
    }
  );
}
...
var SlimArrowDownIcon = Object.assign(
  React15__namespace.default.forwardRef(
...
function useIsRtl(ref) {
  const [isRtl, setIsRtl] = React15.useState(false);
  React15.useEffect(() => {
...
function Popover({
  ...
  const focusedBeforeOpen = React15.useRef(null);
  const [isOpen, setIsOpen] = React15.useState(false);
  const [pos, setPos] = React15.useState(null);
  const [resizeSize, setResizeSize] = React15.useState(null);
  const resizeState = React15.useRef(null);
  const [isDesktop2] = React15.useState(() => isDesktop());
  const containerRef = React15.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React15.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React15.useRef(() => {
  });
  const reposition = React15.useCallback(() => {
  ...
  const focusLast = React15.useCallback(() => {
    if (!popoverRef.current) return;
    const el = getLastFocusable(popoverRef.current);
    (el || popoverRef.current).focus();
  }, []);
  const handleResizeMouseDown = React15.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React15.useMemo(() => {
...
function BusyIndicator({
  ...
  const [isBusy, setIsBusy] = React15.useState(false);
  const labelId = React15.useId();
  const hasChildren = React15__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React15.useRef(null);
  const [animationPhase, setAnimationPhase] = React15.useState("idle");
  const [displayedState, setDisplayedState] = React15.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React15.useState(void 0);
  ...
  const handleKeyUp = React15.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function useIsPhone() {
  const [phone, setPhone] = React15__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React15__namespace.useState(false);
  const dialogId = React15__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React15__namespace.useRef(null);
  const headerRef = React15__namespace.useRef(null);
  const contentRef = React15__namespace.useRef(null);
  const previousActiveElementRef = React15__namespace.useRef(null);
  const returnValueRef = React15__namespace.useRef(void 0);
  const isClosingRef = React15__namespace.useRef(false);
  const didPopRef = React15__namespace.useRef(false);
  const [isDragging, setIsDragging] = React15__namespace.useState(false);
  const [position, setPosition] = React15__namespace.useState(null);
  const positionRef = React15__namespace.useRef(position);
  React15__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React15__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React15__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React15__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React15__namespace.useCallback(() => {
  ...
  const clampPosition = React15__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React15__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React15__namespace.useRef(isPopoverActuallyOpen);
  React15__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React15__namespace.useRef(onAfterClose);
  React15__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React15__namespace.useRef(handleClose);
  React15__namespace.useEffect(() => {
  ...
  const handleMouseDown = React15__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React15__namespace.useCallback(
    (e) => {
  ...
  const handleMouseUp = React15__namespace.useCallback(() => {
    setIsDragging(false);
  }, []);
  const resizeState = React15__namespace.useRef(null);
  const handleResizeMouseDown = React15__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
  ...
  React15__namespace.useImperativeHandle(
    ref,
    () => ({
      show: () => {
        if (controlledOpen === void 0) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "dialog",
    {
      ...
      children: [
        (draggable || resizable) && /* @__PURE__ */ jsxRuntime.jsx(
          "span",
          {
        ...
        (header || headerText) && /* @__PURE__ */ jsxRuntime.jsx(
          "div",
          {
            ref: headerRef,
            className: cn(
            ...
            children: header || /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex items-center gap-3 w-full", children: [
              /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex items-center gap-3 min-w-0", children: [
                state === "Error" && /* @__PURE__ */ jsxRuntime.jsx(MessageErrorIcon, { className: "h-5 w-5 text-sapphire-negative shrink-0" }),
                state === "Success" && /* @__PURE__ */ jsxRuntime.jsx(MessageSuccessIcon, { className: "h-5 w-5 text-sapphire-positive shrink-0" }),
                state === "Warning" && /* @__PURE__ */ jsxRuntime.jsx(MessageWarningIcon, { className: "h-5 w-5 text-sapphire-warning shrink-0" }),
                state === "Information" && /* @__PURE__ */ jsxRuntime.jsx(MessageInformationIcon, { className: "h-5 w-5 text-sapphire-info shrink-0" }),
                showBackButton && state === "None" && /* @__PURE__ */ jsxRuntime.jsx(
                  Button,
                  {
                    design: "SecondaryNeutral",
                    iconOnly: true,
                    icon: /* @__PURE__ */ jsxRuntime.jsx(ArrowLeftIcon, {}),
                ...
                /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex flex-col min-w-0", children: [
                  ...
                  subHeaderText && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "text-sm font-normal text-sapphire-text-tertiary truncate", children: subHeaderText })
                ] })
              ] }),
              headerEndContent && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex items-center gap-2 shrink-0 max-w-[50%] ml-auto", children: headerEndContent })
            ] })
          }
        ),
        /* @__PURE__ */ jsxRuntime.jsx(
        ...
        footer && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex items-center justify-end gap-2 px-6 py-5 shrink-0", children: footer }),
        resizable && !onPhone && /* @__PURE__ */ jsxRuntime.jsx(
          "div",
          {
...
function useIsPhone2() {
  const [phone] = React15.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const isPhone2 = useIsPhone2();
  const popoverRef = React15.useRef(null);
  const dialogRef = React15.useRef(null);
  const handleMobileClose = React15.useCallback(() => {
  ...
  if (isPhone2) {
    ...
    const mobileHeader = header || headerText || showCloseButton ? /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex items-center justify-between w-full", children: [
      header || headerText && /* @__PURE__ */ jsxRuntime.jsx(Title, { level: "H1", id: titleId, className: "text-base font-semibold", children: headerText }),
...
var TabContainerContext = React15__namespace.createContext(null);
function useTabContainerContext() {
  const context = React15.useContext(TabContainerContext);
  if (!context) {
...
function NestedTabItem({
  ...
  const [localDropIndicator, setLocalDropIndicator] = React15.useState(null);
  ...
  const extractTabs = (children) => {
    React15.Children.forEach(children, (child) => {
      ...
      if (child.type === React15__namespace.Fragment) {
  ...
  const handleDragEnter = React15.useCallback((e) => {
    if (!enableDragDrop || isBeingDragged || draggedTabId === props.id) return;
    e.preventDefault();
    e.stopPropagation();
  }, [enableDragDrop, isBeingDragged, draggedTabId, props.id]);
  const handleDragLeave = React15.useCallback((e) => {
  ...
  const handleDropEvent = React15.useCallback((e) => {
    if (!enableDragDrop || isBeingDragged || draggedTabId === props.id) return;
    e.preventDefault();
    e.stopPropagation();
    const rect = e.currentTarget.getBoundingClientRect();
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "relative", children: [
      localDropIndicator === "before" && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute left-2 right-2 top-0 h-0.5 bg-sapphire-text-accent rounded-full z-10" }),
      localDropIndicator === "on" && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute inset-1 border-2 border-sapphire-border-accent border-dashed rounded-md z-10 pointer-events-none bg-sapphire-text-accent/10" }),
      /* @__PURE__ */ jsxRuntime.jsxs(
        "button",
        {
          type: "button",
          ...
          children: [
            props.design && props.design !== "Default" /* Default */ && props.design !== "Neutral" /* Neutral */ && /* @__PURE__ */ jsxRuntime.jsx(
              "span",
              {
                className: cn(
            ...
            props.icon && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex-shrink-0", children: props.icon }),
            /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex-1", children: props.text }),
            props.additionalText && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "text-xs text-sapphire-text-tertiary", children: props.additionalText })
          ]
        }
      ),
      localDropIndicator === "after" && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute left-2 right-2 bottom-0 h-0.5 bg-sapphire-text-accent rounded-full z-10" })
    ] }),
    subTabElements.map((subTab) => /* @__PURE__ */ jsxRuntime.jsx(
      NestedTabItem,
      {
        tab: subTab,
        level: level + 1,
...
function NestedTabsDropdown({
  ...
  const handleNestedDrop = React15.useCallback((_e, tabId, placement) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    ResponsivePopover,
    {
      ...
      footer: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex justify-end px-4 py-2", children: /* @__PURE__ */ jsxRuntime.jsx(Button, { design: "Tertiary", onClick: onClose, children: "Cancel" }) }),
      onClose,
      accessibleRole: "None",
      children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "-mx-4 -my-3 min-w-[200px] max-h-[300px] overflow-auto py-1", children: tabElements.map((tab) => /* @__PURE__ */ jsxRuntime.jsx(
        NestedTabItem,
        {
          tab,
          level: 0,
...
function Tab(props) {
  ...
  const context = React15.useContext(TabContainerContext);
  const [isDropdownOpen, setIsDropdownOpen] = React15.useState(false);
  ...
  const hasSubTabs = !!subTabs && React15.Children.count(subTabs) > 0;
  ...
  const dragOpenTimerRef = React15.useRef(null);
  React15__namespace.useEffect(() => {
  ...
  React15__namespace.useImperativeHandle(ref, () => internalRef.current, []);
  const handleClick2 = React15.useCallback(() => {
    if (disabled) return;
  ...
  const handleExpandClick = React15.useCallback((e) => {
    e.stopPropagation();
    if (!disabled) {
      setIsDropdownOpen((prev) => !prev);
    }
  }, [disabled]);
  const handleSubTabSelect = React15.useCallback((selectedTabId) => {
    if (context) {
      context.selectTab(selectedTabId);
    }
  }, [context]);
  const handleDropdownDrop = React15.useCallback((destinationTabId, placement2) => {
  ...
  const handleKeyUp = React15.useCallback((e) => {
    if (isSingleClickArea) return;
    if (context?.onKeyUp) {
      context.onKeyUp(e, tabId);
    }
  }, [context, tabId, isSingleClickArea]);
  const handleFocus = React15.useCallback(() => {
    if (context?.onTabFocus) {
      context.onTabFocus(tabId);
    }
  }, [context, tabId]);
  const handleBlur = React15.useCallback((e) => {
    if (context?.onTabBlur) {
      context.onTabBlur(e);
    }
  }, [context]);
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { ref: containerRef, className: "fx-tab-item relative inline-flex", children: [
    dropIndicator === "before" && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute left-0 top-1 bottom-1 w-0.5 bg-sapphire-text-accent rounded-full z-10 -translate-x-1" }),
    dropIndicator === "on" && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute inset-0 border-2 border-sapphire-border-accent border-dashed rounded-md z-10 pointer-events-none bg-sapphire-text-accent/10" }),
    /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "inline-flex", children: [
      /* @__PURE__ */ jsxRuntime.jsxs(
        "button",
        {
          ref: internalRef,
          ...
          children: [
            (() => {
              ...
              return /* @__PURE__ */ jsxRuntime.jsx(SemanticIcon, { className: cn("h-3 w-3 flex-shrink-0", colorClass) });
            })(),
            icon && /* @__PURE__ */ jsxRuntime.jsx("span", { className: cn(
              "flex-shrink-0",
              !isStandard && text && !isCollapsed && "mr-2"
            ), children: icon }),
            !isCollapsed && text && /* @__PURE__ */ jsxRuntime.jsx("span", { children: text }),
            isSingleClickArea && !isCollapsed && /* @__PURE__ */ jsxRuntime.jsx(
              SlimArrowDownIcon,
              {
                className: cn(
            ...
            !isCollapsed && additionalText && /* @__PURE__ */ jsxRuntime.jsx(
              "span",
              {
                className: cn(
            ...
            accessibleDescription && /* @__PURE__ */ jsxRuntime.jsx("span", { id: `${tabId}-desc`, className: "sr-only", children: accessibleDescription })
          ]
        }
      ),
      isTwoClickArea && /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "w-px h-4 bg-sapphire-border-primary self-center" }),
        /* @__PURE__ */ jsxRuntime.jsx(
          "button",
          {
            type: "button",
    ...
    dropIndicator === "after" && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "absolute right-0 top-1 bottom-1 w-0.5 bg-sapphire-text-accent rounded-full z-10 translate-x-1" }),
    hasSubTabs && /* @__PURE__ */ jsxRuntime.jsx(
      NestedTabsDropdown,
      {
        subTabs,
...
function useTabDragDrop(options) {
  ...
  const [draggedTabId, setDraggedTabId] = React15.useState(null);
  const [dropTargetId, setDropTargetId] = React15.useState(null);
  const [dropPosition, setDropPosition] = React15.useState(null);
  const dragImageRef = React15.useRef(null);
  const wouldCreateCircle = React15.useCallback(
    (sourceId, targetId) => {
  ...
  const canNestAt = React15.useCallback(
    (targetId) => {
  ...
  const handleDragStart = React15.useCallback(
    (e, tabId) => {
      if (!enabled) return;
  ...
  const getDragProps = React15.useCallback(
  ...
  const getDropIndicator = React15.useCallback(
    (tabId) => {
  ...
  const isDragging = React15.useCallback(
...
function useTabOverflow({
  ...
  const [visibleTabIds, setVisibleTabIds] = React15.useState(tabIds);
  const [startOverflowTabIds, setStartOverflowTabIds] = React15.useState([]);
  const [endOverflowTabIds, setEndOverflowTabIds] = React15.useState([]);
  const [isReady, setIsReady] = React15.useState(false);
  const tabWidthsRef = React15.useRef(/* @__PURE__ */ new Map());
  const hasMeasuredRef = React15.useRef(false);
  const rafId = React15.useRef(null);
  const measureTabs = React15.useCallback(() => {
  ...
  const calculateOverflow = React15.useCallback(() => {
  ...
  const scheduleCalculation = React15.useCallback(() => {
    if (rafId.current) {
  ...
  const recalculate = React15.useCallback(() => {
    hasMeasuredRef.current = false;
...
var ListContext = React15__namespace.createContext(null);
function useOptionalListContext() {
  return React15.useContext(ListContext);
}
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React15.useState(new Set(defaultKeys));
  const previousKeysRef = React15.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React15.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React15.useCallback(() => {
  ...
  const setSelected = React15.useCallback((key, selected) => {
  ...
  const toggleSelection = React15.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React15.useCallback((key) => {
  ...
  const deselectAll = React15.useCallback(() => {
    if (!isControlled) {
...
function useListNavigation({
  ...
  const [focusedIndex, setFocusedIndex] = React15.useState(-1);
  const getAllNavigable = React15.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React15.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const focusItem = React15.useCallback((index) => {
  ...
  const registerItem = React15.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListInner(props) {
  ...
  const growingButtonRef = React15.useRef(null);
  const beforeSentinelRef = React15.useRef(null);
  const afterSentinelRef = React15.useRef(null);
  const [showLoading, setShowLoading] = React15.useState(false);
  const [mediaRange, setMediaRange] = React15.useState("M");
  const itemRegistry = React15.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React15.useState(null);
  const [draggedKey, setDraggedKey] = React15.useState(null);
  const [dropTargetKey, setDropTargetKey] = React15.useState(null);
  const [dropPlacement, setDropPlacement] = React15.useState(null);
  ...
  const itemCount = React15.useMemo(() => {
    if (items) return items.length;
    return React15__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React15.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React15.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React15.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React15.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React15.useCallback((key, e) => {
  ...
  const handleDrop = React15.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React15.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React15.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React15.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
  ...
  const getAllTabbableElements = React15.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React15.useCallback(() => {
  ...
  const enterListAtFocusedItem = React15.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React15.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React15.useMemo(() => {
  ...
  const handleLoadMore = React15.useCallback(() => {
  ...
  const renderedItems = React15.useMemo(() => {
    itemRegistry.current.clear();
    if (items && renderItem) {
      return items.map((item, index) => {
        const key = getItemKey(item, index);
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React15__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React15__namespace.Children.forEach(children, (child) => {
        if (React15__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React15__namespace.memo(ListInner);
...
function useListItemSelection(options) {
  ...
  const shouldForwardTabAfter = React15.useCallback(() => {
  ...
  const shouldForwardTabBefore = React15.useCallback((target) => {
  ...
  const handleClick2 = React15.useCallback((e) => {
    if (disabled) return;
  ...
  const handleSelectionChange = React15.useCallback((checked) => {
    if (disabled) return;
...
function useListItemFocus(options) {
  ...
  const [isFocusedWithin, setIsFocusedWithin] = React15.useState(false);
  const [isActive, setIsActive] = React15.useState(false);
  const getItemIndex = React15.useCallback(() => {
  ...
  const handleFocus = React15.useCallback((e) => {
  ...
  const handleMouseDown = React15.useCallback(() => {
    setIsActive(true);
  }, []);
  const handleMouseUp = React15.useCallback(() => {
    setIsActive(false);
  }, []);
  const handleMouseDownCapture = React15.useCallback(() => {
    setIsFocusedWithin(true);
  }, []);
  const focus = React15.useCallback(() => {
    itemRef.current?.focus();
  }, [itemRef]);
  const blur = React15.useCallback(() => {
...
function useListItemDragDrop(options) {
  ...
  const handleDragEnd = React15.useCallback((e) => {
  ...
  const handleDragLeave = React15.useCallback((_e) => {
  }, []);
  return {
    isDragging,
    isDropTarget,
...
function useListItemRegistration(options) {
  ...
  const listContext = useOptionalListContext();
  const itemIndexRef = React15.useRef(-1);
  React15.useEffect(() => {
...
function CheckBox({
  ...
  const doToggle = React15.useCallback(
    (originalEvent) => {
...
function RadioButton({
  ...
  const descriptionId = `${id}-description`;
  const [internalChecked, setInternalChecked] = React15.useState(defaultChecked);
  const [groupTabIndex, setGroupTabIndex] = React15.useState(null);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const setChecked = React15.useCallback(
    (checked) => {
      if (!isControlled) {
  ...
  const doSelect = React15.useCallback(
    (originalEvent) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "span",
    {
      ...
      children: [
        ...
        showDelete && (deleteButton ? React15__namespace.cloneElement(deleteButton, {
...
function ListItemBase(props) {
  ...
  const listContext = useOptionalListContext();
  const effectiveAccessibleRole = React15.useMemo(() => {
  ...
  const handleDeleteClick = React15.useCallback(() => {
    if (itemKey) {
...
function NavArrowIcon({ className }) {
  return /* @__PURE__ */ jsxRuntime.jsx("svg", { xmlns: "http://www.w3.org/2000/svg", width: "16", height: "16", viewBox: "0 0 16 16", fill: "none", className, children: /* @__PURE__ */ jsxRuntime.jsx("path", { d: "M5.20493 3.23532C5.48927 2.93424 5.96333 2.92076 6.2645 3.20504L10.7645 7.45211C10.9144 7.59365 10.9997 7.79085 10.9998 7.99704C10.9999 8.20338 10.9145 8.40121 10.7645 8.54293L6.2645 12.7949C5.96343 13.0792 5.48937 13.0656 5.20493 12.7646C4.92058 12.4635 4.93419 11.9895 5.2352 11.705L9.15707 7.99704L5.2352 4.29489C4.93413 4.01054 4.92064 3.53648 5.20493 3.23532Z", fill: "currentColor" }) });
}
var highlightBarVariants = cva(
  "absolute left-0 top-0 bottom-0 w-[3px]",
  {
    variants: {
      highlight: {
        ["None" /* None */]: "bg-transparent",
        ["Positive" /* Positive */]: "bg-sapphire-positive",
        ["Critical" /* Critical */]: "bg-sapphire-warning",
        ["Information" /* Information */]: "bg-sapphire-info",
        ["Negative" /* Negative */]: "bg-sapphire-negative"
...
var additionalTextVariants = cva(
  "text-xs shrink-0",
  {
    variants: {
      state: {
...
function ListItem(props) {
  ...
  const handleDetailButtonClick = React15__namespace.useCallback((detail) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    ListItemBase,
    {
      ...
      children: () => /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
        String(highlight) !== "None" && /* @__PURE__ */ jsxRuntime.jsx("span", { className: highlightBarVariants({ highlight }) }),
        /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "flex-1 flex items-center px-[0.75rem] py-[0.5rem] gap-[0.75rem] min-w-0", children: [
          image && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "shrink-0 size-8 max-w-8 max-h-8 overflow-hidden flex items-center justify-center", children: image }),
          displayIcon && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "shrink-0 text-sapphire-text-tertiary", children: icon }),
          /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "flex-1 flex flex-col justify-center min-w-0 gap-0.5", children: [
            /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex items-center gap-2", children: /* @__PURE__ */ jsxRuntime.jsx(
              "span",
              {
                className: cn(
                  "text-sm font-semibold text-sapphire-text-primary",
                  !shouldWrap && "truncate",
            ...
            description && /* @__PURE__ */ jsxRuntime.jsx(
              ...
              {
                className: cn(
                  "text-xs text-sapphire-text-tertiary",
                  !shouldWrap && "truncate",
                  shouldWrap && "break-words"
                ),
          ...
          additionalText && /* @__PURE__ */ jsxRuntime.jsx("span", { className: cn(
            additionalTextVariants({ state: additionalTextState }),
            "shrink-0"
          ), children: additionalText }),
          displayIconEnd && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "shrink-0 text-sapphire-text-tertiary", children: icon }),
          String(type) === "Detail" && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "shrink-0", children: /* @__PURE__ */ jsxRuntime.jsx(
            Button,
            {
              design: "Tertiary" /* Tertiary */,
              iconOnly: true,
          ...
          (action || showNavArrow) && /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "shrink-0 flex items-center gap-[0.5rem]", children: [
            ...
            showNavArrow && /* @__PURE__ */ jsxRuntime.jsx(NavArrowIcon, { className: "text-sapphire-text-tertiary" })
          ] })
        ] })
      ] })
    }
...
function TabOverflowButton({ tabIds, tabs, selectedTabId, onSelect, position, overflowMode, onArrowOut, customTrigger, className, ref }) {
  const [isOpen, setIsOpen] = React15.useState(false);
  const buttonRef = React15.useRef(null);
  React15__namespace.useImperativeHandle(ref, () => buttonRef.current, []);
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    customTrigger && React15.isValidElement(customTrigger) ? React15.cloneElement(customTrigger, {
      ref: buttonRef,
      onClick: handleClick2,
      onKeyDown: handleKeyDown,
      ...
      "button",
      {
        ...
        children: [
          label,
          position === "end" && /* @__PURE__ */ jsxRuntime.jsx(SlimArrowDownIcon, { className: "h-3 w-3" })
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntime.jsx(
      ResponsivePopover,
      {
        ...
        showCloseButton: false,
        contentOnlyOnDesktop: true,
        footer: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex justify-end px-4 py-2", children: /* @__PURE__ */ jsxRuntime.jsx(Button, { design: "Tertiary", onClick: () => setIsOpen(false), children: "Cancel" }) }),
        onClose: () => setIsOpen(false),
        accessibleRole: "None",
        children: /* @__PURE__ */ jsxRuntime.jsx("div", { className: "min-w-[200px] max-h-[300px] overflow-auto", children: /* @__PURE__ */ jsxRuntime.jsx(List, { selectionMode: "None", separators: "None", children: overflowTabs.map((tab) => {
...
function extractNestedTabs(subTabs, tabContents, disabledIds, allTabIds, nestedToParent, parentToChildren, childToDirectParent, parentId, directParentId) {
  React15.Children.forEach(subTabs, (child) => {
    if (!React15.isValidElement(child)) return;
    if (child.type === React15__namespace.Fragment) {
...
function TabContainer(props) {
  ...
  const tabPanelRef = React15.useRef(null);
  const tabRegistrations = React15.useRef(/* @__PURE__ */ new Map());
  const startOverflowButtonRef = React15.useRef(null);
  const endOverflowButtonRef = React15.useRef(null);
  const { tabs, tabIds, allTabIds, tabContents, disabledIds, nestedToParent, parentToChildren, childToDirectParent } = React15.useMemo(
    () => extractTabChildren(children),
    [children]
  );
  const tabDisplayMode = React15.useMemo(() => {
    let hasIcon = false;
    let hasNoIcon = false;
    for (const tab of tabs) {
      if (!React15.isValidElement(tab)) continue;
  ...
  const [internalSelectedTabId, setInternalSelectedTabId] = React15.useState(
    () => {
  ...
  const [focusedTabId, setFocusedTabId] = React15.useState(null);
  const selectedTabId = controlledSelectedTabId ?? internalSelectedTabId;
  const selectedTopLevelTabId = React15.useMemo(() => {
  ...
  const getTabNestingLevel = React15.useCallback(
    (tabId) => {
      let level = 0;
      let currentId = tabId;
  ...
  const getTabChildren = React15.useCallback(
    (tabId) => {
  ...
  const handleTabFocus = React15.useCallback((tabId) => {
    setFocusedTabId(tabId);
  }, []);
  const handleTabBlur = React15.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    if (!tabListRef.current?.contains(relatedTarget)) {
      setFocusedTabId(null);
    }
  }, []);
  const registerTab = React15.useCallback((id, element) => {
    tabRegistrations.current.set(id, { id, element, disabled: disabledIds.has(id) });
  }, [disabledIds]);
  const unregisterTab = React15.useCallback((id) => {
    tabRegistrations.current.delete(id);
  }, []);
  const getTabIndex = React15.useCallback(
  ...
  const renderedTabs = React15.useMemo(() => {
    let tabIndex = 0;
    return React15.Children.map(tabs, (child) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(TabContainerContext.Provider, { value: contextValue, children: /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      className: cn(
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              tabsOverflowMode && overflowReady && hasStartOverflow && /* @__PURE__ */ jsxRuntime.jsx(
                TabOverflowButton,
                {
              ...
              renderedTabs,
              tabsOverflowMode && overflowReady && hasEndOverflow && /* @__PURE__ */ jsxRuntime.jsx(
                TabOverflowButton,
                {
...
function useTabNavigation(options) {
  ...
  const findNextEnabledTab = React15.useCallback(
  ...
  const findFirstEnabledTab = React15.useCallback(() => {
  ...
  const findLastEnabledTab = React15.useCallback(() => {
...
exports.TabContainerBackgroundDesign = TabContainerBackgroundDesign;
exports.TabContainerContext = TabContainerContext;
exports.TabLayout = TabLayout;
exports.TabOverflowButton = TabOverflowButton;
exports.TabOverflowMode = TabOverflowMode;
exports.TabSemanticDesign = TabSemanticDesign;
exports.TabSeparator = TabSeparator;
exports.TabsPlacement = TabsPlacement;
exports.useTabContainerContext = useTabContainerContext;
exports.useTabDragDrop = useTabDragDrop;
