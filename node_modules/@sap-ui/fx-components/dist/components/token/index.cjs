...
function Icon({
  ...
  const handleClick2 = React13.useCallback(
    (e) => {
...
var DeclineIcon = Object.assign(
  React13__namespace.default.forwardRef(
...
function Token({
  ...
  const tokenRef = React13.useRef(null);
  ...
  const handleCloseClick = React13.useCallback(
    (e) => {
      e.stopPropagation();
      onDelete?.({
        originalEvent: e,
  ...
  const handleCloseKeyDown = React13.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        e.stopPropagation();
...
function Popover({
  ...
  const arrowRef = React13.useRef(null);
  const focusedBeforeOpen = React13.useRef(null);
  const [isOpen, setIsOpen] = React13.useState(false);
  const [pos, setPos] = React13.useState(null);
  const [resizeSize, setResizeSize] = React13.useState(null);
  const resizeState = React13.useRef(null);
  const [isDesktop2] = React13.useState(() => isDesktop());
  const containerRef = React13.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React13.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React13.useRef(() => {
  });
  const reposition = React13.useCallback(() => {
  ...
  const handleResizeMouseDown = React13.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React13.useMemo(() => {
...
function BusyIndicator({
  ...
  const [isBusy, setIsBusy] = React13.useState(false);
  const labelId = React13.useId();
  const hasChildren = React13__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React13.useRef(null);
  const [animationPhase, setAnimationPhase] = React13.useState("idle");
  const [displayedState, setDisplayedState] = React13.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React13.useState(void 0);
  ...
  const handleKeyUp = React13.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function useIsPhone() {
  const [phone, setPhone] = React13__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [internalOpen, setInternalOpen] = React13__namespace.useState(defaultOpen);
  const isOpen = controlledOpen ?? internalOpen;
  const onPhone = useIsPhone();
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React13__namespace.useState(false);
  const dialogId = React13__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React13__namespace.useRef(null);
  const headerRef = React13__namespace.useRef(null);
  const contentRef = React13__namespace.useRef(null);
  const previousActiveElementRef = React13__namespace.useRef(null);
  const returnValueRef = React13__namespace.useRef(void 0);
  const isClosingRef = React13__namespace.useRef(false);
  const didPopRef = React13__namespace.useRef(false);
  const [isDragging, setIsDragging] = React13__namespace.useState(false);
  const [position, setPosition] = React13__namespace.useState(null);
  const positionRef = React13__namespace.useRef(position);
  React13__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React13__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React13__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React13__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React13__namespace.useCallback(() => {
  ...
  const clampPosition = React13__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React13__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React13__namespace.useRef(isPopoverActuallyOpen);
  React13__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React13__namespace.useRef(onAfterClose);
  React13__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React13__namespace.useRef(handleClose);
  React13__namespace.useEffect(() => {
  ...
  const handleMouseDown = React13__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const resizeState = React13__namespace.useRef(null);
  const handleResizeMouseDown = React13__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
  ...
  React13__namespace.useImperativeHandle(
    ref,
    () => ({
      show: () => {
        if (controlledOpen === void 0) {
...
function useIsPhone2() {
  const [phone] = React13.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const handleMobileClose = React13.useCallback(() => {
...
var ListContext = React13__namespace.createContext(null);
function useOptionalListContext() {
  return React13.useContext(ListContext);
}
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React13.useState(new Set(defaultKeys));
  const previousKeysRef = React13.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React13.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React13.useCallback(() => {
  ...
  const setSelected = React13.useCallback((key, selected) => {
  ...
  const toggleSelection = React13.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React13.useCallback((key) => {
    return selectedKeys.has(key);
  }, [selectedKeys]);
  const selectAll = React13.useCallback(() => {
  ...
  const deselectAll = React13.useCallback(() => {
    if (!isControlled) {
...
function useListNavigation({
  ...
  const getAllNavigable = React13.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React13.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const focusItem = React13.useCallback((index) => {
  ...
  const registerItem = React13.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListInner(props) {
  ...
  const growingButtonRef = React13.useRef(null);
  const beforeSentinelRef = React13.useRef(null);
  const afterSentinelRef = React13.useRef(null);
  const [showLoading, setShowLoading] = React13.useState(false);
  const [mediaRange, setMediaRange] = React13.useState("M");
  const itemRegistry = React13.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React13.useState(null);
  const [draggedKey, setDraggedKey] = React13.useState(null);
  const [dropTargetKey, setDropTargetKey] = React13.useState(null);
  const [dropPlacement, setDropPlacement] = React13.useState(null);
  ...
  const itemCount = React13.useMemo(() => {
    if (items) return items.length;
    return React13__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React13.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React13.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React13.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React13.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React13.useCallback((key, e) => {
  ...
  const handleDrop = React13.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React13.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React13.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React13.useCallback((e) => {
  ...
  const getAllTabbableElements = React13.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React13.useCallback(() => {
  ...
  const enterListAtFocusedItem = React13.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React13.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React13.useMemo(() => {
  ...
  const handleLoadMore = React13.useCallback(() => {
  ...
  const renderedItems = React13.useMemo(() => {
    itemRegistry.current.clear();
    if (items && renderItem) {
      return items.map((item, index) => {
        const key = getItemKey(item, index);
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React13__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React13__namespace.Children.forEach(children, (child) => {
        if (React13__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React13__namespace.memo(ListInner);
...
function useListItemSelection(options) {
  ...
  const shouldForwardTabAfter = React13.useCallback(() => {
  ...
  const shouldForwardTabBefore = React13.useCallback((target) => {
  ...
  const handleClick2 = React13.useCallback((e) => {
    if (disabled) return;
  ...
  const handleSelectionChange = React13.useCallback((checked) => {
    if (disabled) return;
...
function useListItemFocus(options) {
  ...
  const [isFocusedWithin, setIsFocusedWithin] = React13.useState(false);
  const [isActive, setIsActive] = React13.useState(false);
  const getItemIndex = React13.useCallback(() => {
  ...
  const handleFocus = React13.useCallback((e) => {
  ...
  const handleBlur = React13.useCallback((e) => {
  ...
  const handleMouseDown = React13.useCallback(() => {
    setIsActive(true);
  }, []);
  const handleMouseUp = React13.useCallback(() => {
    setIsActive(false);
  }, []);
  const handleMouseDownCapture = React13.useCallback(() => {
    setIsFocusedWithin(true);
  }, []);
  const focus = React13.useCallback(() => {
    itemRef.current?.focus();
  }, [itemRef]);
  const blur = React13.useCallback(() => {
...
function useListItemDragDrop(options) {
  ...
  const handleDragStart = React13.useCallback((e) => {
    if (!movable || disabled) {
      e.preventDefault();
      return;
    }
  ...
  const handleDragEnd = React13.useCallback((e) => {
    listContext?.onDragEnd?.(id, e);
  }, [id, listContext]);
  const handleDragOver = React13.useCallback((e) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = "move";
    listContext?.onDragOver?.(id, e);
  }, [id, listContext]);
  const handleDrop = React13.useCallback((e) => {
    e.preventDefault();
    listContext?.onDrop?.(id, e);
  }, [id, listContext]);
  const handleDragEnter = React13.useCallback((e) => {
    e.preventDefault();
  }, []);
  const handleDragLeave = React13.useCallback((_e) => {
  }, []);
  return {
    isDragging,
    isDropTarget,
...
function useListItemRegistration(options) {
  ...
  const listContext = useOptionalListContext();
  const itemIndexRef = React13.useRef(-1);
  React13.useEffect(() => {
...
function CheckBox({
  ...
  const [internalChecked, setInternalChecked] = React13.useState(defaultChecked);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const isActive = isChecked || indeterminate;
  const doToggle = React13.useCallback(
    (originalEvent) => {
...
function RadioButton({
  ...
  const descriptionId = `${id}-description`;
  const [internalChecked, setInternalChecked] = React13.useState(defaultChecked);
  const [groupTabIndex, setGroupTabIndex] = React13.useState(null);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const setChecked = React13.useCallback(
    (checked) => {
      if (!isControlled) {
  ...
  const doSelect = React13.useCallback(
    (originalEvent) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "span",
    {
      ...
      children: [
        ...
        showDelete && (deleteButton ? React13__namespace.cloneElement(deleteButton, {
...
function ListItemBase(props) {
  ...
  const listContext = useOptionalListContext();
  const effectiveAccessibleRole = React13.useMemo(() => {
  ...
  const handleDeleteClick = React13.useCallback(() => {
    if (itemKey) {
...
function ListItem(props) {
  ...
  const handleDetailButtonClick = React13__namespace.useCallback((detail) => {
...
var flattenChildren = (nodes) => {
  const result = [];
  React13.Children.forEach(nodes, (child) => {
    if (!React13.isValidElement(child)) return;
    if (child.type === React13__namespace.default.Fragment) {
...
function Tokenizer({
  ...
  const nMoreRef = React13.useRef(null);
  const selectionAnchorRef = React13.useRef(-1);
  const skipNextFocusRef = React13.useRef(false);
  const [focusedIndex, setFocusedIndex] = React13.useState(-1);
  const [selectedIndices, setSelectedIndices] = React13.useState(/* @__PURE__ */ new Set());
  const [visibleCount, setVisibleCount] = React13.useState(null);
  const [internalExpanded] = React13.useState(false);
  const [popoverOpen, setPopoverOpen] = React13.useState(false);
  const [liveAnnouncement, setLiveAnnouncement] = React13.useState("");
  const { t } = reactI18next.useTranslation("fx");
  ...
  const tokenWidthsRef = React13.useRef([]);
  const needsMeasureRef = React13.useRef(true);
  const zeroWidthRetriesRef = React13.useRef(0);
  React13.useEffect(() => {
    needsMeasureRef.current = true;
    zeroWidthRetriesRef.current = 0;
    setVisibleCount(null);
  }, [tokenCount]);
  const measureAndRecalculate = React13.useCallback(() => {
  ...
  const lastParentWidthRef = React13.useRef(0);
  React13.useEffect(() => {
  ...
  const handleNMoreClick = React13.useCallback(() => {
    if (onNMoreClick) {
      onNMoreClick();
    } else {
  ...
  const handleTokenSelect = React13.useCallback(
    (index, detail) => {
      if (disabled) return;
  ...
  const handleTokenDelete = React13.useCallback(
    (index, _detail) => {
      if (disabled) return;
  ...
  const handlePopoverTokenDelete = React13.useCallback(
    (originalIndex) => {
      if (disabled || readonly) return;
  ...
  const handleClearAll = React13.useCallback(() => {
    if (disabled || readonly) return;
  ...
  const scrollTokenIntoView = React13.useCallback((index) => {
  ...
  const handleFocus = React13.useCallback(
    (e) => {
      if (disabled) return;
  ...
  const visibleTokens = tokens.slice(0, effectiveVisible).map((token, index) => {
    return React13.cloneElement(token, {
      key: token.key ?? index,
