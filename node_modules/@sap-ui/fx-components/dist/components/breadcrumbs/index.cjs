...
function Icon({
  ...
  const handleClick2 = React21.useCallback(
    (e) => {
...
var SlimArrowDownIcon = Object.assign(
  React21__namespace.default.forwardRef(
...
function BusyIndicator({
  ...
  const overlayRef = React21.useRef(null);
  const [isBusy, setIsBusy] = React21.useState(false);
  const labelId = React21.useId();
  const hasChildren = React21__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React21.useRef(null);
  const [animationPhase, setAnimationPhase] = React21.useState("idle");
  const [displayedState, setDisplayedState] = React21.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React21.useState(void 0);
  ...
  const handleKeyUp = React21.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
var ListContext = React21__namespace.createContext(null);
function useOptionalListContext() {
  return React21.useContext(ListContext);
}
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React21.useState(new Set(defaultKeys));
  const previousKeysRef = React21.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React21.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React21.useCallback(() => {
  ...
  const setSelected = React21.useCallback((key, selected) => {
  ...
  const toggleSelection = React21.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React21.useCallback((key) => {
    return selectedKeys.has(key);
  }, [selectedKeys]);
  const selectAll = React21.useCallback(() => {
  ...
  const deselectAll = React21.useCallback(() => {
    if (!isControlled) {
...
function useListNavigation({
  ...
  const getAllNavigable = React21.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React21.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const focusItem = React21.useCallback((index) => {
  ...
  const registerItem = React21.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListGrowing({
  ...
  const observerRef = React21.useRef(null);
...
function ListInner(props) {
  ...
  const growingButtonRef = React21.useRef(null);
  const beforeSentinelRef = React21.useRef(null);
  const afterSentinelRef = React21.useRef(null);
  const [showLoading, setShowLoading] = React21.useState(false);
  const [mediaRange, setMediaRange] = React21.useState("M");
  const itemRegistry = React21.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React21.useState(null);
  const [draggedKey, setDraggedKey] = React21.useState(null);
  const [dropTargetKey, setDropTargetKey] = React21.useState(null);
  const [dropPlacement, setDropPlacement] = React21.useState(null);
  ...
  const itemCount = React21.useMemo(() => {
    if (items) return items.length;
    return React21__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React21.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React21.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React21.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React21.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React21.useCallback((key, e) => {
  ...
  const handleDrop = React21.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React21.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React21.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React21.useCallback((e) => {
  ...
  const getAllTabbableElements = React21.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React21.useCallback(() => {
  ...
  const enterListAtFocusedItem = React21.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React21.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React21.useMemo(() => {
  ...
  const handleLoadMore = React21.useCallback(() => {
  ...
  const renderedItems = React21.useMemo(() => {
    itemRegistry.current.clear();
    if (items && renderItem) {
      return items.map((item, index) => {
        const key = getItemKey(item, index);
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React21__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React21__namespace.Children.forEach(children, (child) => {
        if (React21__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React21__namespace.memo(ListInner);
...
function useListItemSelection(options) {
  ...
  const shouldForwardTabAfter = React21.useCallback(() => {
  ...
  const shouldForwardTabBefore = React21.useCallback((target) => {
  ...
  const handleClick2 = React21.useCallback((e) => {
    if (disabled) return;
  ...
  const handleSelectionChange = React21.useCallback((checked) => {
    if (disabled) return;
...
function useListItemFocus(options) {
  ...
  const [isFocusedWithin, setIsFocusedWithin] = React21.useState(false);
  const [isActive, setIsActive] = React21.useState(false);
  const getItemIndex = React21.useCallback(() => {
  ...
  const handleMouseDown = React21.useCallback(() => {
    setIsActive(true);
  }, []);
  const handleMouseUp = React21.useCallback(() => {
    setIsActive(false);
  }, []);
  const handleMouseDownCapture = React21.useCallback(() => {
    setIsFocusedWithin(true);
  }, []);
...
function useListItemDragDrop(options) {
  ...
  const handleDragStart = React21.useCallback((e) => {
    if (!movable || disabled) {
      e.preventDefault();
      return;
    }
  ...
  const handleDragEnd = React21.useCallback((e) => {
  ...
  const handleDragLeave = React21.useCallback((_e) => {
  }, []);
  return {
    isDragging,
    isDropTarget,
...
function useListItemRegistration(options) {
  ...
  const listContext = useOptionalListContext();
  const itemIndexRef = React21.useRef(-1);
  React21.useEffect(() => {
...
function CheckBox({
  ...
  const doToggle = React21.useCallback(
    (originalEvent) => {
...
function RadioButton({
  ...
  const descriptionId = `${id}-description`;
  const [internalChecked, setInternalChecked] = React21.useState(defaultChecked);
  const [groupTabIndex, setGroupTabIndex] = React21.useState(null);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const setChecked = React21.useCallback(
    (checked) => {
      if (!isControlled) {
  ...
  const doSelect = React21.useCallback(
    (originalEvent) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "span",
    {
      ...
      children: [
        ...
        showDelete && (deleteButton ? React21__namespace.cloneElement(deleteButton, {
...
function ListItemBase(props) {
  ...
  const listContext = useOptionalListContext();
  const effectiveAccessibleRole = React21.useMemo(() => {
  ...
  const handleDeleteClick = React21.useCallback(() => {
    if (itemKey) {
...
function useIsRtl(ref) {
  const [isRtl, setIsRtl] = React21.useState(false);
  React21.useEffect(() => {
...
function Popover({
  ...
  const popoverRef = React21.useRef(null);
  const arrowRef = React21.useRef(null);
  const focusedBeforeOpen = React21.useRef(null);
  const [isOpen, setIsOpen] = React21.useState(false);
  const [pos, setPos] = React21.useState(null);
  const [resizeSize, setResizeSize] = React21.useState(null);
  const resizeState = React21.useRef(null);
  const [isDesktop2] = React21.useState(() => isDesktop());
  const containerRef = React21.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React21.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React21.useRef(() => {
  });
  const reposition = React21.useCallback(() => {
  ...
  const focusLast = React21.useCallback(() => {
    if (!popoverRef.current) return;
    const el = getLastFocusable(popoverRef.current);
    (el || popoverRef.current).focus();
  }, []);
  const handleResizeMouseDown = React21.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React21.useMemo(() => {
...
function useIsPhone() {
  const [phone, setPhone] = React21__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React21__namespace.useState(false);
  const dialogId = React21__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React21__namespace.useRef(null);
  const headerRef = React21__namespace.useRef(null);
  const contentRef = React21__namespace.useRef(null);
  const previousActiveElementRef = React21__namespace.useRef(null);
  const returnValueRef = React21__namespace.useRef(void 0);
  const isClosingRef = React21__namespace.useRef(false);
  const didPopRef = React21__namespace.useRef(false);
  const [isDragging, setIsDragging] = React21__namespace.useState(false);
  const [position, setPosition] = React21__namespace.useState(null);
  const positionRef = React21__namespace.useRef(position);
  React21__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React21__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React21__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React21__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React21__namespace.useCallback(() => {
  ...
  const clampPosition = React21__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React21__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React21__namespace.useRef(isPopoverActuallyOpen);
  React21__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React21__namespace.useRef(onAfterClose);
  React21__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React21__namespace.useRef(handleClose);
  React21__namespace.useEffect(() => {
  ...
  const handleMouseDown = React21__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React21__namespace.useCallback(
    (e) => {
  ...
  const handleMouseUp = React21__namespace.useCallback(() => {
    setIsDragging(false);
  }, []);
  const resizeState = React21__namespace.useRef(null);
  const handleResizeMouseDown = React21__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
...
function useIsPhone2() {
  const [phone] = React21.useState(() => isPhone());
  return phone;
}
...
var BreadcrumbsContext = React21.createContext(null);
...
function extractItems(children) {
  ...
  React21.Children.forEach(children, (child) => {
    if (React21.isValidElement(child) && isBreadcrumbsItem(child)) {
...
function injectIndices(children) {
  let idx = 0;
  return React21.Children.map(children, (child) => {
    if (React21.isValidElement(child) && isBreadcrumbsItem(child)) {
      const injected = React21.cloneElement(child, { _index: idx });
      idx++;
      return injected;
    }
    return child;
...
function Breadcrumbs({
  ...
  const olRef = React21.useRef(null);
  const measureRef = React21.useRef(null);
  const overflowBtnRef = React21.useRef(null);
  const itemRefs = React21.useRef(/* @__PURE__ */ new Map());
  const [internalOverflowSize, setInternalOverflowSize] = React21.useState(0);
  const [popoverOpen, setPopoverOpen] = React21.useState(false);
  const [focusedIndex, setFocusedIndex] = React21.useState(-1);
  const [phone] = React21.useState(() => isPhone());
  const allItems = React21.useMemo(() => extractItems(children), [children]);
  ...
  const overflowItems = React21.useMemo(
    () => allItems.slice(0, overflowSize),
    [allItems, overflowSize]
  );
  const visibleItems = React21.useMemo(
  ...
  const linkItems = React21.useMemo(() => {
  ...
  const focusableIds = React21.useMemo(() => {
    const ids = [];
  ...
  const widthCacheRef = React21.useRef([]);
  const overflowBtnWidthRef = React21.useRef(0);
  const measureScheduledRef = React21.useRef(false);
  const measureAndUpdate = React21.useCallback(() => {
  ...
  const fireItemClick = React21.useCallback(
  ...
  const handleLinkClick = React21.useCallback(
    (e, itemProps, itemIndex) => {
      fireItemClick(e, itemProps, itemIndex);
    },
    [fireItemClick]
  );
  const handleCurrentPageClick = React21.useCallback(
    (e, itemProps, itemIndex) => {
      fireItemClick(e, itemProps, itemIndex);
    },
    [fireItemClick]
  );
  const togglePopover = React21.useCallback(() => {
    setPopoverOpen((prev) => !prev);
  }, []);
  const registerItemRef = React21.useCallback(
    (globalIndex, element) => {
  ...
  const moveFocus = React21.useCallback(
    (newIdx) => {
  ...
  const handleOlKeyDown = React21.useCallback(
    (e) => {
  ...
  const handleOlFocusIn = React21.useCallback(
    (e) => {
      const target = e.target;
  ...
  const getTabIndex = React21.useCallback(
    (slotId) => {
      const idx = focusableIds.indexOf(slotId);
  ...
  const indexedChildren = React21.useMemo(() => injectIndices(children), [children]);
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsxs(
          "ol",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntime.jsxs("li", { "data-bc-measure-overflow": true, className: "inline-flex items-center shrink-0", children: [
                /* @__PURE__ */ jsxRuntime.jsxs("span", { className: "inline-flex items-center gap-0.5 text-base px-0.5 py-0.5", children: [
                  /* @__PURE__ */ jsxRuntime.jsx("span", { children: "\u2026" }),
                  /* @__PURE__ */ jsxRuntime.jsx(SlimArrowDownIcon, { className: "h-3 w-3 block" })
                ] }),
                /* @__PURE__ */ jsxRuntime.jsx("span", { className: "mx-1 text-base font-semibold select-none", children: separatorChar })
              ] }),
                ...
                return /* @__PURE__ */ jsxRuntime.jsxs(
                  "li",
                  {
                    ...
                    children: [
                      ...
                      showSep && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "mx-1 text-base font-semibold select-none", children: separatorChar })
                    ]
                  },
                  idx
                );
        ...
        /* @__PURE__ */ jsxRuntime.jsxs(
          "ol",
          {
            ...
            children: [
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntime.jsx(
                      Link,
                      {
                        ...
                        endIcon: /* @__PURE__ */ jsxRuntime.jsx(SlimArrowDownIcon, { className: "h-3 w-3 block", "aria-hidden": "true" }),
                        className: "text-sm px-0.5 py-0.5 gap-0",
                        children: /* @__PURE__ */ jsxRuntime.jsx("span", { "aria-hidden": "true", children: "\u2026" })
                      }
                    ),
                    !allOverflowed && /* @__PURE__ */ jsxRuntime.jsx(
                      "span",
                      {
              ...
              /* @__PURE__ */ jsxRuntime.jsx(BreadcrumbsContext.Provider, { value: contextValue, children: indexedChildren })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntime.jsx(
...
function BreadcrumbsItem({
  ...
  const ctx = React21.useContext(BreadcrumbsContext);
  const linkRef = React21.useRef(null);
