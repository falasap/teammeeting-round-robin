...
// src/types/value-state.ts
var ValueState = /* @__PURE__ */ ((ValueState2) => {
...
function Icon({
  ...
  const handleClick2 = React17.useCallback(
    (e) => {
...
var SysEnter2Icon = Object.assign(
  React17__namespace.default.forwardRef(
...
function BusyIndicator({
  ...
  const overlayRef = React17.useRef(null);
  const [isBusy, setIsBusy] = React17.useState(false);
  const labelId = React17.useId();
  const hasChildren = React17__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React17.useRef(null);
  const [animationPhase, setAnimationPhase] = React17.useState("idle");
  const [displayedState, setDisplayedState] = React17.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React17.useState(void 0);
  ...
  const handleKeyUp = React17.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function InputBase({
  ...
  const [previousValue, setPreviousValue] = React17.useState(defaultValue);
  const [valueOnFocus, setValueOnFocus] = React17.useState(defaultValue);
  const [isFocused, setIsFocused] = React17.useState(false);
  const [isComposing, setIsComposing] = React17.useState(false);
  ...
  const handleCompositionStart = React17.useCallback(() => {
    setIsComposing(true);
  }, []);
  const handleCompositionEnd = React17.useCallback(
    (e) => {
  ...
  const handleClear = React17.useCallback(
    (detail) => {
  ...
  const ariaDescribedBy = React17.useMemo(() => {
    const ids = [];
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            ...
            children: [
              icon && iconPosition === "start" && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex items-center justify-center shrink-0 text-sapphire-text-tertiary", children: icon }),
              /* @__PURE__ */ jsxRuntime.jsx(
                "input",
                {
                  ref: inputRef,
                ...
                "span",
                {
                  ...
                  children: /* @__PURE__ */ jsxRuntime.jsx(
                    Button,
                    {
                      ...
                      iconOnly: true,
                      icon: /* @__PURE__ */ jsxRuntime.jsx(DeclineIcon, { className: size === "Small" /* Small */ ? "h-3 w-3" : "h-4 w-4" }),
                      tabIndex: -1,
                      onClick: handleClear,
              ...
              icon && iconPosition === "end" && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex items-center justify-center shrink-0 text-sapphire-text-tertiary", children: icon })
            ]
          }
        ),
        valueStateMessage && /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            id: messageId,
            ...
            children: [
              ...
              showStateIcon && ValueStateIcon && /* @__PURE__ */ jsxRuntime.jsx(ValueStateIcon, { className: "h-4 w-4 shrink-0 mt-0.5" }),
...
var Input = React17__namespace.default.memo(InputBase);
...
function Popover({
  ...
  const popoverRef = React17.useRef(null);
  const arrowRef = React17.useRef(null);
  const focusedBeforeOpen = React17.useRef(null);
  const [isOpen, setIsOpen] = React17.useState(false);
  const [pos, setPos] = React17.useState(null);
  const [resizeSize, setResizeSize] = React17.useState(null);
  const resizeState = React17.useRef(null);
  const [isDesktop2] = React17.useState(() => isDesktop());
  const containerRef = React17.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React17.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React17.useRef(() => {
  });
  const reposition = React17.useCallback(() => {
  ...
  const focusLast = React17.useCallback(() => {
    if (!popoverRef.current) return;
    const el = getLastFocusable(popoverRef.current);
    (el || popoverRef.current).focus();
  }, []);
  const handleResizeMouseDown = React17.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React17.useMemo(() => {
...
function useIsPhone() {
  const [phone, setPhone] = React17__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [internalOpen, setInternalOpen] = React17__namespace.useState(defaultOpen);
  const isOpen = controlledOpen ?? internalOpen;
  const onPhone = useIsPhone();
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React17__namespace.useState(false);
  const dialogId = React17__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React17__namespace.useRef(null);
  const headerRef = React17__namespace.useRef(null);
  const contentRef = React17__namespace.useRef(null);
  const previousActiveElementRef = React17__namespace.useRef(null);
  const returnValueRef = React17__namespace.useRef(void 0);
  const isClosingRef = React17__namespace.useRef(false);
  const didPopRef = React17__namespace.useRef(false);
  const [isDragging, setIsDragging] = React17__namespace.useState(false);
  const [position, setPosition] = React17__namespace.useState(null);
  const positionRef = React17__namespace.useRef(position);
  React17__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React17__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React17__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React17__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React17__namespace.useCallback(() => {
  ...
  const clampPosition = React17__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React17__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React17__namespace.useRef(isPopoverActuallyOpen);
  React17__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React17__namespace.useRef(onAfterClose);
  React17__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React17__namespace.useRef(handleClose);
  React17__namespace.useEffect(() => {
  ...
  const handleMouseDown = React17__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React17__namespace.useCallback(
    (e) => {
  ...
  const handleMouseUp = React17__namespace.useCallback(() => {
    setIsDragging(false);
  }, []);
  const resizeState = React17__namespace.useRef(null);
  const handleResizeMouseDown = React17__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
  ...
  React17__namespace.useImperativeHandle(
    ref,
    () => ({
      show: () => {
        if (controlledOpen === void 0) {
...
function useIsPhone2() {
  const [phone] = React17.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const popoverRef = React17.useRef(null);
  const dialogRef = React17.useRef(null);
  const handleMobileClose = React17.useCallback(() => {
...
var ListContext = React17__namespace.createContext(null);
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React17.useState(new Set(defaultKeys));
  const previousKeysRef = React17.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React17.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React17.useCallback(() => {
  ...
  const setSelected = React17.useCallback((key, selected) => {
  ...
  const toggleSelection = React17.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React17.useCallback((key) => {
    return selectedKeys.has(key);
  }, [selectedKeys]);
  const selectAll = React17.useCallback(() => {
  ...
  const deselectAll = React17.useCallback(() => {
    if (!isControlled) {
...
function useListNavigation({
  ...
  const getAllNavigable = React17.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React17.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const focusItem = React17.useCallback((index) => {
  ...
  const registerItem = React17.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListInner(props) {
  ...
  const growingButtonRef = React17.useRef(null);
  const beforeSentinelRef = React17.useRef(null);
  const afterSentinelRef = React17.useRef(null);
  const [showLoading, setShowLoading] = React17.useState(false);
  const [mediaRange, setMediaRange] = React17.useState("M");
  const itemRegistry = React17.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React17.useState(null);
  const [draggedKey, setDraggedKey] = React17.useState(null);
  const [dropTargetKey, setDropTargetKey] = React17.useState(null);
  const [dropPlacement, setDropPlacement] = React17.useState(null);
  ...
  const itemCount = React17.useMemo(() => {
    if (items) return items.length;
    return React17__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React17.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React17.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React17.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React17.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React17.useCallback((key, e) => {
  ...
  const handleDrop = React17.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React17.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React17.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React17.useCallback((e) => {
  ...
  const getAllTabbableElements = React17.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React17.useCallback(() => {
  ...
  const enterListAtFocusedItem = React17.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React17.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React17.useMemo(() => {
  ...
  const handleLoadMore = React17.useCallback(() => {
  ...
  const renderedItems = React17.useMemo(() => {
    itemRegistry.current.clear();
    if (items && renderItem) {
      return items.map((item, index) => {
        const key = getItemKey(item, index);
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React17__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React17__namespace.Children.forEach(children, (child) => {
        if (React17__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React17__namespace.memo(ListInner);
...
function highlightText(text, match, highlightClass = "font-semibold bg-sapphire-warning-bg") {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    text.slice(0, index),
    /* @__PURE__ */ jsxRuntime.jsx("span", { className: highlightClass, children: text.slice(index, index + match.length) }),
...
function InputSuggestions({
  ...
  const inputRef = React17.useRef(null);
  const containerRef = React17.useRef(null);
  const popoverContentRef = React17.useRef(null);
  const baseId = React17.useId();
  const listboxId = `${baseId}-listbox`;
  const optionIdPrefix = `${baseId}-option-`;
  const [internalValue, setInternalValue] = React17.useState(defaultValue);
  const [userTypedValue, setUserTypedValue] = React17.useState(defaultValue);
  const [highlightedIndex, setHighlightedIndex] = React17.useState(-1);
  const [hasFocus, setHasFocus] = React17.useState(false);
  const [hasTyped, setHasTyped] = React17.useState(false);
  const [dismissed, setDismissed] = React17.useState(false);
  const [manualOpen, setManualOpen] = React17.useState(false);
  const shouldAutocompleteRef = React17.useRef(true);
  const value = controlledValue ?? internalValue;
  const isControlled = controlledValue !== void 0;
  const filterValue = userTypedValue;
  const flattenedItems = React17.useMemo(() => {
    const result = [];
  ...
  const itemsWithHeaders = React17.useMemo(() => {
    const result = [];
    let lastGroup;
    let itemIndex = 0;
  ...
  const prevOpenRef = React17.useRef(false);
  React17.useEffect(() => {
  ...
  const pendingSelectionRef = React17.useRef(null);
  const previewSuggestion = React17.useCallback(
    (item) => {
  ...
  const restoreTypedValue = React17.useCallback(() => {
    if (!isControlled) {
  ...
  const handleInput = React17.useCallback(
    (newValue) => {
  ...
  const selectItem = React17.useCallback(
    (item) => {
  ...
  const tryAcceptTypeahead = React17.useCallback(() => {
  ...
  const dismissSuggestions = React17.useCallback(() => {
    setDismissed(true);
    setManualOpen(false);
    setHighlightedIndex(-1);
  }, []);
  ...
  const handleItemClick = React17.useCallback(
    (item) => (e) => {
      e.preventDefault();
      selectItem(item);
    },
  ...
  const handlePopoverClose = React17.useCallback(() => {
    dismissSuggestions();
  }, [dismissSuggestions]);
  const srAnnouncement = React17.useMemo(() => {
    if (!isOpen) return "";
  ...
  const renderItem = (item, index) => {
    ...
    return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex items-center gap-2 w-full", children: [
      ...
      /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "flex-1 min-w-0", children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { className: "truncate", children: highlightMatch ? highlightText(item.text, filterValue) : item.text }),
        item.additionalText && /* @__PURE__ */ jsxRuntime.jsx("div", { className: "text-xs text-sapphire-text-tertiary truncate", children: item.additionalText })
      ] })
    ] });
  };
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { ref: containerRef, className: "relative w-full", children: [
    ...
    /* @__PURE__ */ jsxRuntime.jsx("div", { className: "sr-only", "aria-live": "polite", "aria-atomic": "true", children: srAnnouncement }),
    isOpen && /* @__PURE__ */ jsxRuntime.jsx(
      ResponsivePopover,
      {
        ...
        children: /* @__PURE__ */ jsxRuntime.jsx("div", { ref: popoverContentRef, children: filteredItems.length === 0 ? /* @__PURE__ */ jsxRuntime.jsx("div", { className: "px-4 py-2 text-sm text-sapphire-text-tertiary", children: noSuggestionsText }) : /* @__PURE__ */ jsxRuntime.jsx(
          List,
          {
            id: listboxId,
...
exports.InputSuggestions = InputSuggestions;
