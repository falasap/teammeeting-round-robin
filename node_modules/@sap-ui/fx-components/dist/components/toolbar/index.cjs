...
function Icon({
  ...
  const handleClick2 = React14.useCallback(
    (e) => {
  ...
  if (!pathData17 || pathData17.length === 0) {
    return null;
  }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "svg",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx("g", { role: "presentation", children: pathData17.map((path, index) => /* @__PURE__ */ jsxRuntime.jsx("path", { d: path }, index)) })
      ]
    }
  );
}
...
var OverflowIcon = Object.assign(
  React14__namespace.default.forwardRef(
...
function BusyIndicator({
  ...
  const [isBusy, setIsBusy] = React14.useState(false);
  const labelId = React14.useId();
  const hasChildren = React14__namespace.default.Children.count(children) > 0;
...
function Button({
  ...
  const hiddenButtonRef = React14.useRef(null);
  const [animationPhase, setAnimationPhase] = React14.useState("idle");
  const [displayedState, setDisplayedState] = React14.useState(
    null
  );
  const [buttonWidth, setButtonWidth] = React14.useState(void 0);
  ...
  const handleKeyUp = React14.useCallback(
    (e) => {
      if (e.key === "Enter" || e.key === " ") {
...
function Popover({
  ...
  const arrowRef = React14.useRef(null);
  const focusedBeforeOpen = React14.useRef(null);
  const [isOpen, setIsOpen] = React14.useState(false);
  const [pos, setPos] = React14.useState(null);
  const [resizeSize, setResizeSize] = React14.useState(null);
  const resizeState = React14.useRef(null);
  const [isDesktop2] = React14.useState(() => isDesktop());
  const containerRef = React14.useRef(null);
  const isRtl = useIsRtl(containerRef);
  const getOpenerEl = React14.useCallback(() => resolveOpener(opener), [opener]);
  const doCloseRef = React14.useRef(() => {
  });
  const reposition = React14.useCallback(() => {
  ...
  const handleResizeMouseDown = React14.useCallback(
    (e) => {
  ...
  const resizeHandleCorner = React14.useMemo(() => {
...
var ToolbarContext = React14.createContext({
  isInOverflow: false,
  closeOverflow: noop
});
var useToolbarContext = () => React14.useContext(ToolbarContext);
...
var flattenChildren = (nodes) => {
  const result = [];
  React14.Children.forEach(nodes, (child) => {
    if (!React14.isValidElement(child)) return;
    if (child.type === React14__namespace.default.Fragment) {
      result.push(
        ...flattenChildren(
...
function startsWithTitle(child) {
  ...
  if (typeName === "ToolbarItem") {
    const inner = React14.Children.toArray(
      child.props.children
    );
    return inner.some(
      (c) => React14.isValidElement(c) && (c.type?.displayName ?? c.type?.name) === "Title"
    );
  }
  return false;
}
...
function Toolbar({
  ...
  const toolbarRef = React14.useRef(null);
  const measureRef = React14.useRef(null);
  const overflowBtnRef = React14.useRef(null);
  const [containerWidth, setContainerWidth] = React14.useState(0);
  const [itemWidths, setItemWidths] = React14.useState([]);
  const [overflowOpen, setOverflowOpen] = React14.useState(false);
  React14.useImperativeHandle(
    ref,
    () => ({
      focus() {
  ...
  const flatItems = React14.useMemo(() => flattenChildren(children), [children]);
  const descriptionId = React14.useId();
  React14.useEffect(() => {
  ...
  const overflowedSet = React14.useMemo(() => {
  ...
  const closeOverflow = React14.useCallback(() => {
    setOverflowOpen(false);
  }, []);
  const handleOverflowClick = React14.useCallback(() => {
  ...
  const getFocusableItems = React14.useCallback(() => {
    const el = toolbarRef.current;
    if (!el) return [];
    return Array.from(
      el.querySelectorAll(
  ...
  if (isInOverflow) {
    return /* @__PURE__ */ jsxRuntime.jsxs(
      "button",
      {
        ...
        children: [
          icon && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-flex items-center shrink-0 [&>svg]:h-4 [&>svg]:w-4", children: icon }),
          text && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex-1", children: text }),
          text == null && tooltip && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex-1", children: tooltip }),
          endIcon && /* @__PURE__ */ jsxRuntime.jsx("span", { className: "inline-flex items-center shrink-0 [&>svg]:h-4 [&>svg]:w-4", children: endIcon })
        ]
      }
    );
  }
  const isIconOnly = !!icon && !text;
  return /* @__PURE__ */ jsxRuntime.jsx("div", { ref, className: "shrink-0", style: width ? { width } : void 0, children: /* @__PURE__ */ jsxRuntime.jsx(
    Button,
    {
      design,
      size: "Medium" /* Medium */,
...
function useIsPhone() {
  const [phone, setPhone] = React14__namespace.useState(() => isPhone());
...
function Dialog({
  ...
  const [isPopoverActuallyOpen, setIsPopoverActuallyOpen] = React14__namespace.useState(false);
  const dialogId = React14__namespace.useId();
  const stableId = id || dialogId;
  const dialogRef = React14__namespace.useRef(null);
  const headerRef = React14__namespace.useRef(null);
  const contentRef = React14__namespace.useRef(null);
  const previousActiveElementRef = React14__namespace.useRef(null);
  const returnValueRef = React14__namespace.useRef(void 0);
  const isClosingRef = React14__namespace.useRef(false);
  const didPopRef = React14__namespace.useRef(false);
  const [isDragging, setIsDragging] = React14__namespace.useState(false);
  const [position, setPosition] = React14__namespace.useState(null);
  const positionRef = React14__namespace.useRef(position);
  React14__namespace.useLayoutEffect(() => {
    positionRef.current = position;
  }, [position]);
  const dragStartRef = React14__namespace.useRef({ x: 0, y: 0, dialogX: 0, dialogY: 0 });
  const effectiveRole = React14__namespace.useMemo(() => {
    if (state === "Error" /* Error */ || state === "Warning" /* Warning */) {
      return "alertdialog";
    }
    return accessibleRole || "dialog";
  }, [state, accessibleRole]);
  const effectiveAriaDescribedBy = React14__namespace.useMemo(() => {
    const parts = [];
  ...
  const ensureDirectPositioning = React14__namespace.useCallback(() => {
  ...
  const clampPosition = React14__namespace.useCallback(
    (x, y) => {
      const dialog = dialogRef.current;
  ...
  const handleClose = React14__namespace.useCallback((escPressed) => {
    const dialog = dialogRef.current;
    if (!dialog) return;
  ...
  const isPopoverActuallyOpenRef = React14__namespace.useRef(isPopoverActuallyOpen);
  React14__namespace.useEffect(() => {
    isPopoverActuallyOpenRef.current = isPopoverActuallyOpen;
  }, [isPopoverActuallyOpen]);
  const onAfterCloseRef = React14__namespace.useRef(onAfterClose);
  React14__namespace.useEffect(() => {
    onAfterCloseRef.current = onAfterClose;
  }, [onAfterClose]);
  const handleCloseRef = React14__namespace.useRef(handleClose);
  React14__namespace.useEffect(() => {
  ...
  const handleMouseDown = React14__namespace.useCallback(
    (e) => {
      if (!draggable) return;
  ...
  const handleMouseMove = React14__namespace.useCallback(
    (e) => {
  ...
  const handleMouseUp = React14__namespace.useCallback(() => {
    setIsDragging(false);
  }, []);
  const resizeState = React14__namespace.useRef(null);
  const handleResizeMouseDown = React14__namespace.useCallback(
    (e) => {
      if (!resizable) return;
      if (stretch && onPhone) return;
      e.preventDefault();
  ...
  React14__namespace.useImperativeHandle(
    ref,
    () => ({
      show: () => {
        if (controlledOpen === void 0) {
...
function useIsPhone2() {
  const [phone] = React14.useState(() => isPhone());
  return phone;
}
function ResponsivePopover({
  ...
  const handleMobileClose = React14.useCallback(() => {
...
var ListContext = React14__namespace.createContext(null);
function useOptionalListContext() {
  return React14.useContext(ListContext);
}
function useListSelection({
  ...
  const [internalKeys, setInternalKeys] = React14.useState(new Set(defaultKeys));
  const previousKeysRef = React14.useRef(new Set(defaultKeys));
  const isControlled = controlledKeys !== void 0;
  const selectedKeys = isControlled ? new Set(controlledKeys) : internalKeys;
  const getItemByKey = React14.useCallback((key) => {
    if (!items || !getItemKey) return void 0;
    return items.find((item, index) => getItemKey(item, index) === key);
  }, [items, getItemKey]);
  const getSelectedItems = React14.useCallback(() => {
  ...
  const setSelected = React14.useCallback((key, selected) => {
  ...
  const toggleSelection = React14.useCallback((key) => {
    setSelected(key, !selectedKeys.has(key));
  }, [selectedKeys, setSelected]);
  const isSelected = React14.useCallback((key) => {
  ...
  const deselectAll = React14.useCallback(() => {
    if (!isControlled) {
...
function useListNavigation({
  ...
  const getAllNavigable = React14.useCallback(() => {
  ...
  const getAllNavigableExcludingGrowingButton = React14.useCallback(() => {
    if (!containerRef.current) return [];
    const elements = Array.from(
      containerRef.current.querySelectorAll(
  ...
  const handleKeyDown = React14.useCallback((e) => {
    const key = e.key;
    if (key === "Tab") {
      return;
    }
  ...
  const registerItem = React14.useCallback((_index, _element) => {
  }, []);
  return {
    focusedIndex,
    setFocusedIndex,
...
function ListInner(props) {
  ...
  const growingButtonRef = React14.useRef(null);
  const beforeSentinelRef = React14.useRef(null);
  const afterSentinelRef = React14.useRef(null);
  const [showLoading, setShowLoading] = React14.useState(false);
  const [mediaRange, setMediaRange] = React14.useState("M");
  const itemRegistry = React14.useRef(/* @__PURE__ */ new Map());
  const [previouslyFocusedItemKey, setPreviouslyFocusedItemKey] = React14.useState(null);
  const [draggedKey, setDraggedKey] = React14.useState(null);
  const [dropTargetKey, setDropTargetKey] = React14.useState(null);
  const [dropPlacement, setDropPlacement] = React14.useState(null);
  ...
  const itemCount = React14.useMemo(() => {
    if (items) return items.length;
    return React14__namespace.Children.count(children);
  }, [items, children]);
  ...
  const registerItem = React14.useCallback((key, index) => {
    itemRegistry.current.set(key, index);
  }, []);
  const unregisterItem = React14.useCallback((key) => {
    itemRegistry.current.delete(key);
  }, []);
  const handleDragStart = React14.useCallback((key, _e) => {
    setDraggedKey(key);
  }, []);
  const handleDragEnd = React14.useCallback((_key, _e) => {
    setDraggedKey(null);
    setDropTargetKey(null);
    setDropPlacement(null);
  }, []);
  const handleDragOver = React14.useCallback((key, e) => {
  ...
  const handleDrop = React14.useCallback((targetKey, e) => {
  ...
  const handleForwardTabAfter = React14.useCallback((key, event) => {
  ...
  const handleForwardTabBefore = React14.useCallback((key, event) => {
    setPreviouslyFocusedItemKey(key);
  ...
  const handleBeforeSentinelFocus = React14.useCallback((e) => {
  ...
  const getAllTabbableElements = React14.useCallback(() => {
    return Array.from(
      document.querySelectorAll(
        'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]'
      )
  ...
  const focusNextAfterList = React14.useCallback(() => {
  ...
  const enterListAtFocusedItem = React14.useCallback(() => {
  ...
  const handleAfterSentinelFocus = React14.useCallback((e) => {
    const relatedTarget = e.relatedTarget;
    const isFromGrowingButton = relatedTarget?.hasAttribute("data-growing-button");
  ...
  const roleSupportsAriaSelected = React14.useMemo(() => {
  ...
  const handleLoadMore = React14.useCallback(() => {
  ...
  const renderedItems = React14.useMemo(() => {
    itemRegistry.current.clear();
    if (items && renderItem) {
      return items.map((item, index) => {
        const key = getItemKey(item, index);
        itemRegistry.current.set(key, index);
        return /* @__PURE__ */ jsxRuntime.jsx(React14__namespace.Fragment, { children: renderItem(item, index) }, key);
      });
    }
    if (children) {
      let index = 0;
      React14__namespace.Children.forEach(children, (child) => {
        if (React14__namespace.isValidElement(child) && child.props && typeof child.props.itemKey === "string") {
...
var List = React14__namespace.memo(ListInner);
...
function useMessageHeight(messageRef, valueStateMessage) {
  const [messageHeight, setMessageHeight] = React14.useState(0);
...
function useAnnounce() {
  const [announcement, setAnnouncement] = React14.useState({ message: "", mode: "polite" });
  const timeoutRef = React14.useRef(void 0);
  const announce = React14.useCallback((text, mode = "polite") => {
...
function useTypeAhead(options, onSelect) {
  const [, setTypedChars] = React14.useState("");
  const timeoutRef = React14.useRef(void 0);
...
function Select({
  ...
  const triggerRef = React14.useRef(null);
  const messageRef = React14.useRef(null);
  const [portalTarget, setPortalTarget] = React14.useState(null);
  ...
  const initialSelectedIndex = React14.useMemo(() => {
  ...
  const [internalSelectedIndex, setInternalSelectedIndex] = React14.useState(initialSelectedIndex);
  const [internalOpen, setInternalOpen] = React14.useState(defaultOpen);
  const [focusedIndex, setFocusedIndex] = React14.useState(-1);
  const [previousSelectedIndex, setPreviousSelectedIndex] = React14.useState(initialSelectedIndex);
  const isOpen = controlledOpen ?? internalOpen;
  ...
  const selectedOption = React14.useMemo(() => {
  ...
  const openPopover = React14.useCallback(() => {
  ...
  const closePopover = React14.useCallback(() => {
    if (controlledOpen === void 0) {
      setInternalOpen(false);
    }
    setFocusedIndex(-1);
  ...
  const handleBeforeClose = React14.useCallback(
    (detail) => {
  ...
  const togglePopover = React14.useCallback(() => {
    if (isOpen) {
      closePopover();
    } else {
      openPopover();
  ...
  const renderDisplayText = () => {
    ...
    if (!selectedOption) {
      return /* @__PURE__ */ jsxRuntime.jsx("span", { className: "text-sapphire-text-tertiary", children: "Select..." });
    }
    ...
    return /* @__PURE__ */ jsxRuntime.jsx("span", { children: selectedOption.text });
  };
  const renderChildren = React14.useCallback(() => {
    return React14.Children.map(children, (child, index) => {
      if (!React14.isValidElement(child)) return child;
      const data = getOptionData(child);
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { ref, className: cn("relative w-full", isIconOnly && "w-auto", className), children: [
    ...
    /* @__PURE__ */ jsxRuntime.jsx(
      "div",
      {
        ...
        children: isIconOnly ? /* @__PURE__ */ jsxRuntime.jsx("span", { className: "flex items-center justify-center text-sapphire-text-tertiary", children: icon }) : /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
          /* @__PURE__ */ jsxRuntime.jsx("div", { className: "flex-1 py-2 text-sm truncate min-w-0", children: renderDisplayText() }),
          ...
          ), children: /* @__PURE__ */ jsxRuntime.jsx(SlimArrowDownIcon, { className: size === "Medium" /* Medium */ ? "h-3 w-3" : "h-4 w-4" }) })
        ] })
      }
    ),
    valueState !== "None" /* None */ && valueStateMessage && /* @__PURE__ */ jsxRuntime.jsx("span", { id: descriptionId, className: "sr-only", children: valueStateMessage }),
    accessibleDescription && /* @__PURE__ */ jsxRuntime.jsx("span", { id: `${baseId}-desc-text`, className: "sr-only", children: accessibleDescription }),
    /* @__PURE__ */ jsxRuntime.jsx(
      SelectPopover,
      {
        open: isOpen,
    ...
    /* @__PURE__ */ jsxRuntime.jsx("div", { ...liveRegionProps })
  ] });
}
function ToolbarSelect({
  ...
  if (isInOverflow) {
    return /* @__PURE__ */ jsxRuntime.jsx("div", { ref, className: cn("w-full px-3 py-1", className), children: /* @__PURE__ */ jsxRuntime.jsx(
      Select,
      {
        value,
        disabled,
...
function useListItemSelection(options) {
  ...
  const shouldForwardTabAfter = React14.useCallback(() => {
  ...
  const shouldForwardTabBefore = React14.useCallback((target) => {
  ...
  const handleClick2 = React14.useCallback((e) => {
    if (disabled) return;
  ...
  const handleSelectionChange = React14.useCallback((checked) => {
    if (disabled) return;
...
function useListItemFocus(options) {
  ...
  const [isFocusedWithin, setIsFocusedWithin] = React14.useState(false);
  const [isActive, setIsActive] = React14.useState(false);
  const getItemIndex = React14.useCallback(() => {
  ...
  const handleFocus = React14.useCallback((e) => {
  ...
  const handleBlur = React14.useCallback((e) => {
  ...
  const handleMouseDownCapture = React14.useCallback(() => {
    setIsFocusedWithin(true);
  }, []);
  const focus = React14.useCallback(() => {
    itemRef.current?.focus();
  }, [itemRef]);
  const blur = React14.useCallback(() => {
...
function useListItemDragDrop(options) {
  ...
  const handleDragEnd = React14.useCallback((e) => {
  ...
  const handleDragEnter = React14.useCallback((e) => {
    e.preventDefault();
  }, []);
  const handleDragLeave = React14.useCallback((_e) => {
  }, []);
  return {
    isDragging,
    isDropTarget,
...
function useListItemRegistration(options) {
  ...
  const listContext = useOptionalListContext();
  const itemIndexRef = React14.useRef(-1);
  React14.useEffect(() => {
...
function CheckBox({
  ...
  const [internalChecked, setInternalChecked] = React14.useState(defaultChecked);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const isActive = isChecked || indeterminate;
  const doToggle = React14.useCallback(
    (originalEvent) => {
...
function RadioButton({
  ...
  const descriptionId = `${id}-description`;
  const [internalChecked, setInternalChecked] = React14.useState(defaultChecked);
  const [groupTabIndex, setGroupTabIndex] = React14.useState(null);
  const isControlled = controlledChecked !== void 0;
  const isChecked = isControlled ? controlledChecked : internalChecked;
  const setChecked = React14.useCallback(
    (checked) => {
      if (!isControlled) {
  ...
  const doSelect = React14.useCallback(
    (originalEvent) => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "span",
    {
      ...
      children: [
        ...
        showDelete && (deleteButton ? React14__namespace.cloneElement(deleteButton, {
...
function ListItemBase(props) {
  ...
  const listContext = useOptionalListContext();
  const effectiveAccessibleRole = React14.useMemo(() => {
  ...
  const handleDeleteClick = React14.useCallback(() => {
    if (itemKey) {
...
function ListItem(props) {
  ...
  const handleDetailButtonClick = React14__namespace.useCallback((detail) => {
...
function Option({
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    ListItem,
    {
      ...
      text: displayText,
      icon: icon ? /* @__PURE__ */ jsxRuntime.jsx("span", { className: "text-current", children: icon }) : void 0,
      additionalText,
      tooltip,
      selected,
      disabled,
...
function ToolbarSelectOption({ value, selected, children, ref }) {
  return /* @__PURE__ */ jsxRuntime.jsx(Option, { ref, value, selected, children });
}
function ToolbarItem({ overflowPriority: _overflowPriority, preventOverflowClosing: _preventOverflowClosing, children, className, ref }) {
  const { isInOverflow } = useToolbarContext();
  if (isInOverflow) {
    return /* @__PURE__ */ jsxRuntime.jsx("div", { ref, className: cn("w-full px-3 py-2", className), children });
  }
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
